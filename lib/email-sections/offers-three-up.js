...
function render(props = {}) {
  ...
  const cells = offers
    .map((o) => `<td valign="top" width="33.33%" class="tg-stack" style="vertical-align:top;padding:0 6px;">${renderCard(o, props.ctaText || "View")}</td>`)
    .join("");

  // If fewer than 3, pad with empty <td>s of equal width to preserve grid
  const filler = "<td width=\"33.33%\" class=\"tg-stack\" style=\"display:none\"></td>".repeat(Math.max(0, 3 - offers.length));
