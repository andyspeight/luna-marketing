...
function render(props = {}, brand) {
  ...
  const stepsHtml = steps.map((d, i) => renderStep(d, i, brand)).filter(Boolean).join("");
  ...
  <table role="presentation" border="0" cellspacing="0" cellpadding="0" width="100%" style="border-top:1px solid ${DESIGN_TOKENS.border};padding-top:24px;">
    <tr>
      <td style="padding:0 0 12px 0;">
        <div style="font-family:${FONTS.heading};font-size:${titleSize}px;font-weight:700;letter-spacing:-0.015em;color:${DESIGN_TOKENS.textPrimary};line-height:${Math.round(titleSize * (22 / 16))}px;margin:0;">${title}</div>
      </td>
    </tr>
    ${stepsHtml}
  </table>
