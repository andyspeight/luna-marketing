...
function renderItem(item, index, brand, editable) {
  ...
  <td style="padding:5px 0;">
    <table role="presentation" border="0" cellspacing="0" cellpadding="0" width="100%">
      <tr>
        ...
        <td valign="top">
          <span${editAttr(editable, `items.${index}.text`)} style="font-family:${FONTS.body};font-size:13px;color:${DESIGN_TOKENS.textSecondary};line-height:21px;">${text}</span>
        </td>
      </tr>
    </table>
  </td>
...
function render(props = {}, brand, editable) {
  ...
  if (useTwoColumns) {
    ...
    const leftRows = leftItems.map((it, i) => renderItem(it, i, brand, editable)).filter(Boolean).join("");
    const rightRows = rightItems
      .map((it, i) => renderItem(it, i + half, brand, editable))
      .filter(Boolean)
      .join("");
    ...
    const rows = items.map((d, i) => renderItem(d, i, brand, editable)).filter(Boolean).join("");
...
<tr><td bgcolor="${DESIGN_TOKENS.bgSecondary}" class="tg-pad-mobile" style="background-color:${DESIGN_TOKENS.bgSecondary};padding:20px 36px;border-bottom:1px solid ${DESIGN_TOKENS.border};">
  <div${editAttr(editable, "label")} style="font-family:${FONTS.body};font-size:${labelSize}px;font-weight:700;letter-spacing:0.12em;text-transform:uppercase;color:${DESIGN_TOKENS.accent};line-height:${Math.round(labelSize * (14 / 10))}px;margin:0 0 10px 0;">${label}</div>
  ${bodyHtml}
</td></tr>`;
}
