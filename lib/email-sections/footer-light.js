...
// Brevo merge tags for legally-required compliance links. When a user
// leaves the URL blank in the editor, we fall back to Brevo's template
// language so each recipient gets a working personalized link at send
// time. Reference: Brevo Template Language docs (help.brevo.com).
//
//   {{ unsubscribe }}  — per-recipient unsubscribe URL, hosted on Brevo's
//                        compliance-validated unsub page. Mandatory by law
//                        for marketing emails (UK GDPR, CAN-SPAM, CASL).
//   {{ mirror }}       — view-this-email-in-browser URL.
//
// Match labels case-insensitively because the user might type "Unsubscribe",
// "unsubscribe", "unsubscribe from this list", etc.
...
function renderManageLinks(links, brand) {
  ...
  // Always emit at least an unsubscribe link, even if `links` is empty or
  // malformed. Legal requirement for marketing emails.
  ...
  const parts = workingList
    .map((l) => {
      ...
      //   1. If the URL is already a Brevo merge tag (e.g. {{ unsubscribe }})
      //      pass it through unchanged. Brevo will replace it at send time.
      //   2. Else run safeUrl() on the user-supplied URL.
      //   3. Else fall back to a default merge tag for known labels
      //      (Unsubscribe → {{ unsubscribe }}, View in browser → {{ mirror }}).
      let url = "";
      const rawUrl = l.url ? String(l.url).trim() : "";
      if (/^\{\{\s*(unsubscribe|mirror|update_profile|update_preferences)\s*\}\}$/.test(rawUrl)) {
        // It's a Brevo merge tag literal — pass through. safeUrl would
        // strip the {{ }} braces and reject the "URL".
        url = rawUrl;
      } else {
        url = safeUrl(l.url);
        if (!url) {
