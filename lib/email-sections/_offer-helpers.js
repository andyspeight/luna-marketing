...
// Image cell with bombproof fallback. Always renders a coloured div as the
// background, then layers an <img> on top. If the image URL fails to load,
// the alt text and broken-image icon don't appear because the image's own
// dimensions are zero — the background div fills the space.
//
// The colour is derived deterministically from the destination so two offers
// don't get identical placeholders, and the background also shows briefly
// while the real image is loading.
function imageCell({ offer, height = 200, width = "100%", radius = 0 }) {
  ...
  const radiusStyle = radius ? `border-radius:${radius}px;` : "";

  // Deterministic gradient from the offer's destination
  ...
  // No URL at all — just the gradient div
  if (!url) {
    return `<div style="background:${grad};${radiusStyle}height:${height}px;width:${width};display:block;"></div>`;
  }

  // URL present — wrap in a coloured div so if the image fails (CORS,
  // 404, expired Unsplash URL, etc.) the cell still has a presence.
  // The <img> uses `display:block` and zero alt text styling so a
  // broken-image icon won't show. We also use a tiny transparent 1x1
  // PNG as the alt-content fallback for clients that strip the alt.
  return `<div style="background:${grad};${radiusStyle}height:${height}px;width:${width};line-height:0;font-size:0;overflow:hidden;">
<img src="${url}" alt="${alt}" width="600" class="tg-img-fluid" style="display:block;width:${width};max-width:600px;height:${height}px;object-fit:cover;${radiusStyle}border:0;outline:none;text-decoration:none;color:transparent;font-size:0;line-height:0;" onerror="this.style.opacity=0">
</div>`;
}
...
// ── Demo-data banner ──────────────────────────────────────────────────
//
// Rendered at the top of the FIRST offer section in any contiguous run
// of demo-data sections. The dedupe logic in email-render-prepare.js
// sets props._suppressDemoBanner on subsequent demo sections so two
// banners don't appear back to back.
//
// Strip the banner system entirely in production by setting
// DEMO_OFFERS_DISABLED=true on Vercel — that turns off the demo
// fallback so isDemoSet() always returns false.
function demoBanner(offers, opts) {
