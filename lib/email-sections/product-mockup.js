      ...
      <tr>
        ...
        <td valign="top" align="right">
          <table role="presentation" border="0" cellspacing="0" cellpadding="0">
            <tr>
              <td bgcolor="${DESIGN_TOKENS.accent}"${editAttr(editable, `messages.${index}.text`)} style="background-color:${DESIGN_TOKENS.accent};border-radius:14px 14px 4px 14px;padding:10px 14px;font-family:${FONTS.body};font-size:${msgSize}px;font-weight:500;line-height:1.45;color:${DESIGN_TOKENS.primaryDark};">${text}</td>
            </tr>
          </table>
        </td>
      </tr>
    ...
    <table role="presentation" border="0" cellspacing="0" cellpadding="0" width="100%">
      <tr>
        <td valign="top" align="left">
          <table role="presentation" border="0" cellspacing="0" cellpadding="0">
            <tr>
              <td bgcolor="${DESIGN_TOKENS.bgTertiary}"${editAttr(editable, `messages.${index}.text`)} style="background-color:${DESIGN_TOKENS.bgTertiary};border-radius:14px 14px 14px 4px;padding:10px 14px;font-family:${FONTS.body};font-size:${msgSize}px;line-height:1.45;color:${DESIGN_TOKENS.textPrimary};">${text}</td>
            </tr>
          </table>
        </td>
        <td width="15%">&nbsp;</td>
...
function renderChatVariant(props, brand, editable) {
  const { DESIGN_TOKENS, FONTS } = brand || STATIC_BRAND;
  ...
  const messagesHtml = messages.map((d, i) => renderChatMessage(d, brand, editable, i)).filter(Boolean).join("");
    ...
    <td bgcolor="${DESIGN_TOKENS.primary}" style="background-color:${DESIGN_TOKENS.primary};padding:14px 18px;border-radius:16px 16px 0 0;">
      <table role="presentation" border="0" cellspacing="0" cellpadding="0" width="100%">
        <tr>
          ...
          <td valign="middle">
            <div${editAttr(editable, "header_name")} style="font-family:${FONTS.body};font-size:${headerNameSize}px;font-weight:600;color:#ffffff;line-height:${Math.round(headerNameSize * (18 / 13))}px;margin:0;">${headerName}</div>
...
// ── Dashboard variant ───────────────────────────────────────────────
function renderDashboardRow(row, brand, editable, index) {
  const { DESIGN_TOKENS, FONTS } = brand || STATIC_BRAND;
...
<tr>
  <td style="padding:14px 16px;border-bottom:1px solid ${DESIGN_TOKENS.borderLight};">
    <table role="presentation" border="0" cellspacing="0" cellpadding="0" width="100%">
      <tr>
        <td valign="middle" align="left"${editAttr(editable, `dashboard_rows.${index}.label`)} style="font-family:${FONTS.body};font-size:13px;color:${DESIGN_TOKENS.textSecondary};line-height:18px;">${label}</td>
...
function renderDashboardVariant(props, brand, editable) {
  const { DESIGN_TOKENS, FONTS } = brand || STATIC_BRAND;
  ...
  const rowsHtml = rows.map((d, i) => renderDashboardRow(d, brand, editable, i)).filter(Boolean).join("");
...
<table role="presentation" border="0" cellspacing="0" cellpadding="0" width="440" align="center" style="background-color:#ffffff;border-radius:12px;border:1px solid #e2e8f0;max-width:440px;margin:0 auto;overflow:hidden;">
  <tr>
    <td bgcolor="#ffffff" style="background-color:#ffffff;padding:16px 18px;border-bottom:1px solid ${DESIGN_TOKENS.border};">
      <div${editAttr(editable, "dashboard_title")} style="font-family:${FONTS.heading};font-size:${titleSize}px;font-weight:700;letter-spacing:-0.01em;color:${DESIGN_TOKENS.textPrimary};line-height:${Math.round(titleSize * (18 / 14))}px;">${title}</div>
    </td>
  </tr>
...
function render(props = {}, brand, editable) {
  ...
  const inner = variant === "dashboard"
    ? renderDashboardVariant(props, brand, editable)
    : renderChatVariant(props, brand, editable);
