...
// Shared modules — single source of truth for date/season context and content
// validation, reused from the social generation pipeline so email and social
// stay aligned and cannot drift apart (the failure mode that bit us before).
const { getDateContext } = require("../api/b2b-prompt.js");
...
# HOW TO WRITE AS ANDY / TRAVELGENIX — THE VOICE THAT MATTERS MOST

The laws above keep the email safe and factual. This section is how it must
SOUND. An email can be factually perfect and still fail if it does not sound
like Travelgenix.

## The core philosophy

Travelgenix is a SOLUTION PROVIDER for the travel industry. We operate in
travel technology, but we NEVER talk like a tech company. We do not care about
the plumbing and neither does the reader. NEVER explain how the technology
works under the hood — no APIs, integrations, "platforms", "infrastructure",
"systems". Talk only about what the solution DOES for the business owner:
their bottom line, their sanity, their growth.
...
technology. We SELL the technology. The villain is always admin, faff,
slowness, the lost evening, the booking that slipped away. NEVER the tools.
The winning frame: great service and great technology are the same thing. The
right tech puts the brilliant human front and centre with the heavy lifting
done.

## Scratch the Itch (the structure for problem-led content)

When an email section is built around a business problem, follow this rhythm:
identify a real day-to-day headache, agitate it just enough that the reader
nods along ("yes, someone gets it"), then offer the solution smoothly as the
natural answer. Headache, then recognition, then relief. Never problem then
product then hard sell. When you describe the solution, say only what the
person gets back (time, calm, the booking saved), never how it works.
...
WRONG: "Travelgenix offers a scalable, cutting-edge software infrastructure."
RIGHT: "We fix the daily headaches that slow your business down, so you have
the room to actually grow."

WRONG: "Click here to schedule a comprehensive demonstration."
RIGHT: "Let's take a look at how we can sort this out for your business."

## The temperature

Relaxed, warm, friendly and supportive. A knowledgeable friend who has seen
the problem before and quietly knows how to fix it. NOT a loud pub bore. No
forced matey-ness, no "mate", no "look", no staged anecdotes. Professional
with the jacket off, not professional after six pints.
...
function buildDataContextLayer(context) {
  ...
  sortedProducts.forEach(p => {
    ...
    // Product Brain — the daily-refreshed knowledge layer. Authority order:
    // the owner's Supplement is ground truth, then the canonical copy above,
    // then the auto-scraped "latest from site" summary (useful but lower trust).
    if (f["Supplement"]) {
      lines.push(`- Owner supplement (AUTHORITATIVE, trust over everything else): ${String(f["Supplement"]).replace(/\s+/g, " ").trim()}`);
    }
    if (f["Brain Summary"]) {
      lines.push(`- Latest from site (auto-scraped, use for current facts; do not override the owner supplement or invent beyond it): ${String(f["Brain Summary"]).replace(/\s+/g, " ").trim().slice(0, 1200)}`);
    }
    lines.push("");
  });
 ...
 * @returns {string}
 */
function buildDateContextLayer() {
  const d = getDateContext();
  return [
    "# TODAY'S DATE AND THE SELLING SEASON",
    "",
    `Today is ${d.todayStr}. The current year is ${d.year}.`,
    `Right now, UK travel agents are actively selling: ${d.sellingNow}.`,
    "",
    "RULES ON DATES:",
    `- NEVER reference a past year as if it is upcoming. It is ${d.year}. Writing "summer ${d.year - 1}" as a future event is wrong and makes the sender look out of touch.`,
    "- Anchor any seasonal angle to what is actually being sold right now (above).",
    "- Genuine historical references are fine. The rule is only about not framing a PAST year as the UPCOMING season.",
  ].join("\n");
}
...
// DEPRECATED (22 May 2026): runCoreValidation now uses the shared
// validateContent (api/validate-content.js) as the single source of truth for
// banned words and competitor names. These two arrays are no longer used by
// validation and are kept only for backwards-compatible exports. Do not add to
// them — update the shared validator instead.
 ...
 *
 * Converged 22 May 2026 onto the shared validateContent (api/validate-content.js)
 * so email inherits exactly the same factual safeguards as social: the
 * stale-year FAIL, metaphor-aware banned words (landscape/ecosystem),
 * fabricated-stat/name/quote patterns, and competitor detection. We keep TWO
 * email-specific behaviours on top: (1) the em/en dash is a HARD reject for
 * email (stricter than social, which is correct for a sent email), and (2) the
 * numeric-claims heads-up. This removes the divergent banned list that would
 * otherwise drift from the social one over time.
 */
function runCoreValidation(text) {
  ...
  // Em dash AND en dash — HARD BAN, email-specific (stricter than social).
  ...
  // Shared validator — single source of truth for everything else. Maps its
  // issue codes into the email composer's hardErrors/warnings shape.
  //   - STALE_YEAR, COMPETITOR_NAMED, FABRICATED_* → hardErrors (must not send)
  //   - banned words/phrases, Oxford comma, curly quotes → warnings (review)
  const shared = validateContent(text);
  shared.issues.forEach(issue => {
    const isHard =
      issue.code === "STALE_YEAR" ||
      issue.code === "COMPETITOR_NAMED" ||
      issue.code === "FABRICATED_CLIENT" ||
      issue.code === "FABRICATED_CASE_STUDY" ||
      issue.code === "FABRICATED_STAT" ||
      issue.code === "FABRICATED_AWARD" ||
      issue.code === "FABRICATED_PARTNERSHIP";
    if (isHard) {
      hardErrors.push(`${issue.code}: ${issue.detail}. Composer output rejected.`);
    } else {
      warnings.push(`${issue.code}: ${issue.detail}`);
    }
  });

  // Number-like claims that look hallucinated (percentages and "Nx faster").
  // Kept as an email-specific heads-up in addition to the shared checks.
