...
function getBrand(clientKit) {
  if (!clientKit || typeof clientKit !== "object") {
    return {
      ...
      COMPANY,
      BRAND,
      // Empty FOOTER + SOCIALS so section files can safely read these
      // without null-checking. When no clientKit is passed, the renderer
      // falls back to hardcoded COMPANY values inside the section files.
      FOOTER: {
        reason: "",
        legalLines: [],
        companyAddress: "",
  ...
  return {
    ...
    // FOOTER — content fields used by every footer-* section as a fallback
    // when the section's own props are empty. Sourced from the client's
    // My Settings → Footer & Legal card (via brand-kit.js).
    FOOTER: {
      reason: clientKit.footerReason || "",
      legalLines: Array.isArray(clientKit.footerLegalLines) ? clientKit.footerLegalLines : [],
    ...
    // SOCIALS — pre-assembled [{platform, url}] array from brand-kit's
    // buildSocialsArray(). Empty array if the client has no social URLs
    // set, in which case footer sections suppress their social row.
    SOCIALS: Array.isArray(clientKit.socials) ? clientKit.socials : [],
    BRAND,
    BRAND_KIT: clientKit,
  };
}
