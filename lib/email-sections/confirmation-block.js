...
function render(props = {}, brand, editable) {
  ...
  const headlineSize = scaleFont(28, props.headline_size);
...
<tr><td bgcolor="${DESIGN_TOKENS.bgPrimary}" class="tg-pad-mobile" style="background-color:${DESIGN_TOKENS.bgPrimary};padding:48px 32px 32px;">
  ...
  <div${editAttr(editable, "eyebrow")} style="font-family:${FONTS.body};font-size:${eyebrowSize}px;font-weight:700;letter-spacing:0.12em;text-transform:uppercase;color:${eyebrowColor};margin:0 0 8px 0;">${eyebrow}</div>
  <div${editAttr(editable, "headline")} style="font-family:${FONTS.heading};font-size:${headlineSize}px;font-weight:700;line-height:${Math.round(headlineSize * (34 / 28))}px;letter-spacing:-0.025em;color:${DESIGN_TOKENS.textPrimary};margin:0 0 12px 0;">${headline}</div>
  ${text ? `<div${editAttr(editable, "text")} style="font-family:${FONTS.body};font-size:${textSize}px;line-height:${Math.round(textSize * (24 / 15))}px;color:${DESIGN_TOKENS.textSecondary};margin:0;">${text}</div>` : ""}
</td></tr>`;
}
...
const schema = {
  ...
  fields: [
    ...
    { key: "text_size", label: "Supporting copy text size", type: "fontSize", optional: true },
  ],
};
