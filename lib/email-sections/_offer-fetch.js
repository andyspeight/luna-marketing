...
// Field allowlist for the auto-fetch query. Anything else in props.fetch
// is ignored — a block can't smuggle arbitrary keys into the upstream
// payload.
const ALLOWED_FETCH_KEYS = new Set([
  "type",            // "Accommodation" | "Flight" | "Package" | "Any"
  ...
  "sort",            // "price:asc" | "price:desc" | "departure:asc" etc.
  "destinations",    // array of destination filters per Travelify spec
  "origins",         // array of origin airports
  "boardBasis",      // array
  "minRating",       // number
  "minNights",
  "maxNights",
...
function pickAllowed(obj) {
  ...
  for (const k of Object.keys(obj)) {
    if (ALLOWED_FETCH_KEYS.has(k)) out[k] = obj[k];
  }
  return out;
}
 ...
 *
 * @param {Object} fetchParams - Filters per Travelify spec (allowlisted)
...
async function fetchOffers(fetchParams = {}, limit = 6) {
  ...
  // Always set sensible defaults — same shape the widget uses
  const payload = {
    type: "Accommodation",
    deduping: "Aggressive",
    currency: "GBP",
    ...
    sort: "price:asc",
    ...safeParams,
    // maxOffers always reflects our limit, regardless of what was passed
    maxOffers: safeLimit,
  };
  ...
  try {
    ...
    // The Travelify response shape (per the widget) is { results: [...] }
    // wrapped in proxy passthrough. Defensive about both shapes.
    const rawOffers =
...
function normaliseOffer(o) {
  ...
  // Prefer accommodation summary fields when present, fall back to flight
  ...
  return {
    ...
    // Pricing — prefer the formatted strings (Travelify already includes
    // currency symbols + thousands separators), fall back to numeric.
...
module.exports = { fetchOffers, normaliseOffer };
