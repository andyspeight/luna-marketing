...
Return ONLY valid JSON, no markdown fences:
{
  ...
  "content": "Full HTML using <h2> <h3> <p> <ol> <li> <strong>. No <h1>. No inline styles.",
  "excerpt": "2-3 sentence social excerpt",
...
async function generateBlogPost({ tenantId, product, client }) {
  ...
  let blog;
  try { blog = JSON.parse(raw.replace(/```json/gi, "").replace(/```/g, "").trim()); }
  catch (e) { throw new Error("Blog output failed to parse: " + e.message); }

  const title = String(blog.title || p.name).slice(0, 200);
  const content = String(blog.content || "");
  if (!content.trim()) throw new Error("Blog returned empty content");
  ...
  if (!heldByValidator) {
    try {
      ...
      const slug = (duda && (duda.slug || duda.post_slug)) || String(blog.slug || "");
      liveUrl = slug ? `https://travelgenix.io/blog/${slug}` : "";
      published = true;
    } catch (e) {
      console.error("[promo] Duda publish failed (non-fatal):", e.message);
    }
  }
  ...
  const fields = {
    ...
    [PQ.li]: blog.linkedInTeaser || blog.excerpt || "",
  ...
  if (heldByValidator) fields["Quality Issues"] = v.formattedReport.slice(0, 50000);
  ...
  return { ok: true, title, published, held: heldByValidator, url: liveUrl };
}
