...
const { escHtml, renderBody, scaleFont, editAttr } = require("./_helpers");
...
function render(props = {}, brand, editable) {
  ...
  if (subhead) {
    inner += `<div${editAttr(editable, "subhead")} style="font-family:${FONTS.body};font-size:${subheadSize}px;color:${BRAND.inkMuted};line-height:${Math.round(subheadSize * (22 / 15))}px;text-align:${align};margin:0 0 12px 0;">${subhead}</div>`;
  }
  if (bodyHtml) {
    inner += `<div${editAttr(editable, "body")} style="font-family:${FONTS.body};font-size:${bodySize}px;color:${BRAND.inkSoft};line-height:${Math.round(bodySize * (26 / 16))}px;text-align:${align};margin:0;">${bodyHtml}</div>`;
  }
