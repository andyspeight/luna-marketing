...
function renderItem(item, brand, editable, prefix) {
  ...
  return `
${tag ? `<div${editAttr(editable, `${prefix}.tag`)} style="font-family:${FONTS.body};font-size:10px;font-weight:700;letter-spacing:0.12em;text-transform:uppercase;color:${DESIGN_TOKENS.accent};line-height:14px;margin:0 0 8px 0;">${tag}</div>` : ""}
<div${editAttr(editable, `${prefix}.headline`)} style="font-family:${FONTS.heading};font-size:${headlineSize}px;font-weight:700;line-height:1.3;letter-spacing:-0.015em;color:${DESIGN_TOKENS.textPrimary};margin:0 0 8px 0;">${headline}</div>
${body ? `<p${editAttr(editable, `${prefix}.body`)} style="font-family:${FONTS.body};font-size:${bodySize}px;line-height:1.6;color:${DESIGN_TOKENS.textSecondary};margin:0 0 12px 0;">${body}</p>` : ""}
${linkUrl ? `<a href="${linkUrl}"${editAttr(editable, `${prefix}.link_text`)} style="font-family:${FONTS.body};font-size:13px;font-weight:600;color:${ctaFg};text-decoration:none;display:inline-block;line-height:18px;">${linkText}</a>` : ""}`.trim();
}

function render(props = {}, brand, editable) {
  const { DESIGN_TOKENS, FONTS } = brand || STATIC_BRAND;

  const left = renderItem(props.left, brand, editable, "left");
  const right = renderItem(props.right, brand, editable, "right");
