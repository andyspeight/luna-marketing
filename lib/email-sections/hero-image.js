...
function render(props = {}, brand, editable) {
  ...
  // Author-written text colours. Default keeps the original treatment (white
  // headline, softened-white deck on the navy panel) so existing emails are
...
<tr><td bgcolor="${DESIGN_TOKENS.primary}" style="background-color:${DESIGN_TOKENS.primary};padding:0;font-size:0;line-height:0;">
  ...
  const deckHtml = deck
    ? `<div${editAttr(editable, "deck")} style="font-family:${FONTS.body};font-size:${deckSize}px;font-weight:400;line-height:${Math.round(deckSize * (25 / 16))}px;color:${bodyColour};margin:0 0 28px 0;max-width:480px;letter-spacing:-0.005em;">${deck}</div>`
    : "";
...
<tr><td bgcolor="${DESIGN_TOKENS.primary}" class="tg-pad-mobile" style="background-color:${DESIGN_TOKENS.primary};padding:48px 32px 40px;color:#ffffff;">
  ${eyebrowHtml}
  <div${editAttr(editable, "headline")} style="font-family:${FONTS.heading};font-size:${headlineSize}px;font-weight:700;line-height:${Math.round(headlineSize * (42 / 36))}px;letter-spacing:-0.025em;color:${headingColour};margin:0 0 16px 0;">${headline}</div>
  ${deckHtml}
  ${ctaHtml}
</td></tr>`;
}
...
const schema = {
  ...
  fields: [
    ...
    { key: "heading_colour", label: "Headline colour", type: "colour", optional: true, defaultPreview: "#FFFFFF", placeholder: "Defaults to current headline colour", help: "Colour of the headline text. Leave empty to keep the default." },
    ...
    { key: "body_colour", label: "Subhead / deck colour", type: "colour", optional: true, defaultPreview: "#E6EBF5", placeholder: "Defaults to current deck colour", help: "Colour of the deck text. Leave empty to keep the default." },
