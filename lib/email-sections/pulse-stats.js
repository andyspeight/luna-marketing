...
function renderStat(stat, brand) {
  ...
  return `
<div style="font-family:${FONTS.heading};font-size:32px;font-weight:800;letter-spacing:-0.03em;color:#ffffff;line-height:1;margin:0 0 6px 0;">${value || "&nbsp;"}</div>
<div style="font-family:${FONTS.body};font-size:12px;color:rgba(255,255,255,0.7);line-height:1.4;margin:0;">${label}</div>`.trim();
}
...
function render(props = {}, brand) {
  ...
  const padded = [stats[0], stats[1], stats[2]].map((s) => renderStat(s, brand) || "&nbsp;");
...
<tr><td bgcolor="${DESIGN_TOKENS.primary}" class="tg-pad-mobile" style="background-color:${DESIGN_TOKENS.primary};padding:36px;color:#ffffff;">
  <div style="font-family:${FONTS.body};font-size:${labelSize}px;font-weight:700;letter-spacing:0.12em;text-transform:uppercase;color:${DESIGN_TOKENS.accentLight};line-height:${Math.round(labelSize * (14 / 11))}px;margin:0 0 6px 0;">${label}</div>
  ${title ? `<div style="font-family:${FONTS.heading};font-size:${titleSize}px;font-weight:700;letter-spacing:-0.02em;color:#ffffff;line-height:1.2;margin:0 0 24px 0;">${title}</div>` : `<div style="height:24px;line-height:24px;font-size:24px;">&nbsp;</div>`}
  ${statsHtml}
  ${source ? `
  <div style="border-top:1px solid rgba(255,255,255,0.1);padding-top:14px;margin-top:20px;">
    <p style="font-family:${FONTS.body};font-size:11px;color:rgba(255,255,255,0.5);line-height:1.5;margin:0;">${source}</p>
  </div>` : ""}
</td></tr>`;
}
