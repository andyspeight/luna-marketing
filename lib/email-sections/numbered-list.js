...
const { escHtml, fallback, inlineMarkdown, scaleFont, editAttr, richAttr } = require("./_helpers");

function renderItem(item, index, isLast, brand, editable) {
  ...
  <td style="padding:16px 0;${borderStyle}">
    <table role="presentation" border="0" cellspacing="0" cellpadding="0" width="100%">
      <tr>
        ...
        <td valign="top">
          ${title ? `<div${editAttr(editable, `items.${index}.title`)} style="font-family:${FONTS.heading};font-size:14px;font-weight:600;letter-spacing:-0.01em;color:${DESIGN_TOKENS.textPrimary};line-height:21px;margin:2px 0 4px 0;">${title}</div>` : ""}
          ${text ? `<div${editAttr(editable, `items.${index}.text`)}${richAttr(editable)} style="font-family:${FONTS.body};font-size:13px;line-height:1.55;color:${DESIGN_TOKENS.textSecondary};margin:0;">${text}</div>` : ""}
        </td>
      </tr>
    </table>
  </td>
...
function render(props = {}, brand, editable) {
  ...
  const headerHtml = `
    <table role="presentation" border="0" cellspacing="0" cellpadding="0" width="100%">
      <tr>
        <td${title ? editAttr(editable, "title") : ""} valign="baseline" align="left" style="font-family:${FONTS.heading};font-size:${titleSize}px;font-weight:700;letter-spacing:-0.02em;color:${DESIGN_TOKENS.textPrimary};line-height:1.2;">${title || "&nbsp;"}</td>
        ${meta ? `<td${editAttr(editable, "meta")} valign="baseline" align="right" style="font-family:${FONTS.body};font-size:11px;color:${DESIGN_TOKENS.textTertiary};font-weight:500;line-height:1.4;">${meta}</td>` : ""}
      </tr>
    </table>`;

  const itemsHtml = items
    .map((item, i) => renderItem(item, i, i === items.length - 1, brand, editable))
    .filter(Boolean)
    .join("");
