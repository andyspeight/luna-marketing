...
function renderItem(item, index, brand) {
...
function render(props = {}, brand) {
  ...
  if (useTwoColumns) {
    ...
    const leftRows = leftItems.map((it, i) => renderItem(it, i, brand)).filter(Boolean).join("");
    const rightRows = rightItems
      .map((it, i) => renderItem(it, i + half, brand))
      .filter(Boolean)
      .join("");
    ...
    const rows = items.map((d, i) => renderItem(d, i, brand)).filter(Boolean).join("");
