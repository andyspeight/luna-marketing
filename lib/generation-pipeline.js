...
const { polishPosts } = require("./voice-editor.js");
...
// ─────────────────────────────────────────────────────────────
// Voice editor pass (LAYER between writer and validator)
// ─────────────────────────────────────────────────────────────

/**
 * Run the second-pass voice editor over an array of normalized posts. Uses the
 * pipeline's own callModel so the editor inherits the guardrail-sentinel
 * assertion and model config. NEVER throws — on failure, originals pass through.
 *
 * IMPORTANT: callers must run validateAndTagPosts AFTER this. The validator is
 * the final gate; the editor improves voice but the validator still guards
 * against any fabrication the editor might introduce while polishing.
 *
 * @param {object[]} posts - normalized posts from parsePostsJson
 * @returns {Promise<{posts: object[], reports: object[], summary: object}>}
 */
async function polishPostsThroughEditor(posts) {
  // The editor's system prompt carries the sentinel, so callModel's assertion
  // passes. We pass our own callModel so the editor reuses the same client.
  return polishPosts(posts, callModel);
}
