...
function renderLineItem(item, isLast) {
...
function render(props = {}) {
  ...
  const lineItemsHtml = lineItems.map((item, i) =>
    renderLineItem(item, i === lineItems.length - 1)
  ).join("");
  const totalHtml = renderTotalRow(total);
  const paymentHtml = renderPaymentRow(payment);
