...
function render(props = {}, brand, editable) {
  ...
  // Mesh glow. Previously an absolutely-positioned 600x600 <div> — Gmail
  // strips position/transform/filter styles, so that div fell into normal
  // flow as a ~600px-tall empty navy block above the content (a huge blank
  // box in real sends that never showed in the browser preview). The glow
  // now lives in the td's background-image stack instead: clients that
  // support multiple backgrounds get the radial glow over the gradient,
  // everything else falls back to the gradient or flat bgcolor. Nothing
  // exists in the layout flow to break.
  return `
<tr><td bgcolor="${bg}" class="tg-pad-mobile" align="center" style="background-color:${bg};background-image:radial-gradient(circle at 50% 50%, rgba(0,180,216,0.18) 0%, transparent 55%), linear-gradient(135deg, ${bg} 0%, ${bg2} 100%);padding:60px 40px;color:#ffffff;text-align:center;">
