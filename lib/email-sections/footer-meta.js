...
function render(props = {}, brand) {
  ...
  // Resolve brand-kit fallbacks for the legal lines + reason. Section
  // props win when populated, brand.FOOTER (sourced from My Settings →
  // Footer & Legal) fills gaps. With brand-kit fallback this section's
  // `lines` is effectively optional even though the schema says required.
  const brandFooter = (brand && brand.FOOTER) || {};

  const propLines = Array.isArray(props.lines) ? props.lines.filter(l => String(l || "").trim()) : [];
  const lines = propLines.length > 0
    ? props.lines
    : (Array.isArray(brandFooter.legalLines) ? brandFooter.legalLines : []);
