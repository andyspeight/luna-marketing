...
<tr><td bgcolor="${DESIGN_TOKENS.primary}" class="tg-pad-mobile" align="center" style="background-color:${DESIGN_TOKENS.primary};background-image:linear-gradient(135deg, ${DESIGN_TOKENS.primary} 0%, ${DESIGN_TOKENS.primaryLight} 100%);padding:48px 32px;text-align:center;color:#ffffff;">
  <div style="font-family:${FONTS.heading};font-size:${titleSize}px;font-weight:700;letter-spacing:-0.02em;line-height:${Math.round(titleSize * (30 / 24))}px;color:#ffffff;margin:0 0 12px 0;">${title}</div>
  ${text ? `<div style="font-family:${FONTS.body};font-size:${textSize}px;line-height:${Math.round(textSize * (24 / 15))}px;color:rgba(255,255,255,0.8);margin:0 auto 24px auto;max-width:420px;">${text}</div>` : `<div style="height:24px;line-height:24px;font-size:24px;">&nbsp;</div>`}
  ${ctaHtml}
</td></tr>`;
}
