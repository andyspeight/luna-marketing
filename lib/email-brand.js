...
function getBrand(clientKit) {
  ...
  // Build SOCIALS array from the client's social URL fields. Mirrors
  // buildSocialsArray() in brand-kit.js — kept self-contained here so
  // email-brand.js doesn't pull in brand-kit's fetch/auth dependencies.
  // Only includes platforms with a real URL. Order matches the footer
  // social icon convention (linkedin, facebook, instagram, x, youtube,
  // tiktok). Footer renders this when its own socials array is empty.
  const socialMap = [
  ...
  const SOCIALS = socialMap
    .map(({ key, platform }) => {
      const url = typeof clientKit[key] === "string" ? clientKit[key].trim() : "";
      return url ? { platform, url } : null;
    })
    .filter(Boolean);

  // Build FOOTER block — reason line, legal lines array, company address,
  // ABTA/ATOL numbers, support hours/phone. Footer sections read this
  // when their own equivalent props are empty.
  const FOOTER = {
    reason: clientKit.footerReason || "",
    legalLines: Array.isArray(clientKit.footerLegalLines)
      ? clientKit.footerLegalLines.filter(Boolean)
      : [],
    companyAddress: clientKit.companyAddress || "",
    abtaNumber: clientKit.abtaNumber || "",
    atolNumber: clientKit.atolNumber || "",
    supportHours: clientKit.supportHours || "",
    supportPhone: clientKit.supportPhone || "",
    supportPhoneRaw: clientKit.supportPhoneRaw || "",
  };
  ...
  return {
    ...
    COMPANY: {
      ...
      address: clientKit.companyAddress || COMPANY.address,
