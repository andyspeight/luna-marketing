...
// Computes the current date context and what UK travel agents are actively
// SELLING right now. Injected into the prompt so the model never references a
// past year as if upcoming (e.g. "summer 2025" when it is 2026) and always
// frames seasons against the real calendar. Computed at runtime so it can
// never go stale. Added 22 May 2026 after a generated post referenced
// "summer 2025" as upcoming.
function getDateContext(now) {
  const d = now || new Date();
  const year = d.getFullYear();
  const month = d.getMonth(); // 0 = Jan
  const monthNames = ["January","February","March","April","May","June","July","August","September","October","November","December"];
  const todayStr = `${d.getDate()} ${monthNames[month]} ${year}`;
  const nextYear = year + 1;

  // The UK travel sales calendar — what agents are actively selling this quarter.
  let sellingNow;
  if (month <= 2) {
    // Jan–Mar
    sellingNow = `summer ${year} (this is peak booking season — the main event), plus late winter-sun deals for the next few weeks`;
  } else if (month <= 5) {
    // Apr–Jun
    sellingNow = `peak summer ${year}, the early winter-sun ${year}/${String(nextYear).slice(2)} season starting to open, AND the lates market — late availability for imminent summer ${year} departures, which is a bigger and bigger part of the business`;
  } else if (month <= 8) {
    // Jul–Sep
    sellingNow = `winter sun for ${year}/${nextYear}, next year's summer ${nextYear}, AND the lates market — last-minute summer ${year} breaks, which is a growing and important selling window`;
  } else {
    // Oct–Dec
    sellingNow = `winter ${year}/${nextYear}, Christmas and New Year ${year}, and next summer ${nextYear}`;
  }

  return { todayStr, year, nextYear, sellingNow };
}
...
═══════════════════════════════════════════════════════════
TODAY'S DATE AND THE SELLING SEASON — GET THIS RIGHT
═══════════════════════════════════════════════════════════

Today is ${dateCtx.todayStr}. The current year is ${dateCtx.year}.

Right now, UK travel agents are actively selling: ${dateCtx.sellingNow}.

HARD RULES ON DATES AND SEASONS:
- NEVER reference a past year as if it is upcoming. It is ${dateCtx.year}. Writing
  "summer ${dateCtx.year - 1}" or "booking for ${dateCtx.year - 1}" as a future
  event is WRONG and makes Andy look out of touch. If you mean the upcoming
  summer, it is summer ${dateCtx.year} (or summer ${dateCtx.nextYear} if you are
  clearly talking about next year's bookings).
- When you write about "this season", "the season ahead", "early bookings" or
  "what agents are selling", anchor it to what is ACTUALLY being sold right now
  (above), not a generic or remembered season.
- Only use a specific year when you are confident it is correct. A timeless post
  ("agents who prepare early win") is safer than a post with a wrong year.
- Genuine historical references are fine ("demand has climbed since 2019").
  The rule is only about not framing a PAST year as the UPCOMING season.
...
- Bookable Websites with a library of 100+ widgets
- Widget Suite (NEW launch): 40 brand-new widgets joining that library.
  Get the referent right: the platform has 100+ widgets in total; the NEW
  Widget Suite launch adds 40 new ones. Never attribute "100+" to the new
  suite, and never describe the platform total as 40.
...
module.exports = { buildB2BSystemPrompt, getNextMonday, getDateContext };
