...
P8. THE WEBSITE MOCKUP IS THE WHOLE IMAGE.
    When a brief describes a product mockup over a host website (e.g. "Luna Chat anchored to the bottom-right of a travel agency website"), the entire canvas is filled by the website-plus-widget composition. Do NOT add eyebrow labels, headlines, body copy, feature bullets, marketing taglines, or any descriptive text in the canvas margins or beside the mockup. The image is purely the visual asset.

    Marketing copy belongs in the email's text sections that wrap this image, not painted onto the image itself. If the email needs a headline like "Intelligent Travel Chat", that lives in the email body, written by the composer, not in the hero image.

    If the canvas is wider than the natural website mockup needs to be, scale the website mockup UP to fill the canvas (longer hero banner, wider navigation row, more destination cards across, larger search bar) rather than leaving empty space that gets filled with text. The mockup should always feel deliberately composed at the canvas dimensions — never undersized with copy padding the gaps.

    This rule applies to every product-mockup brief, not just Luna Chat.
