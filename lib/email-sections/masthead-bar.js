...
function render(props = {}, brand, editable) {
  ...
  // Wordmark size — base 18px in the markup below.
  const wordmarkSize = scaleFont(18, props.wordmark_size);
  ...
  const wordmarkHtml = linkUrl
    ? `<a href="${linkUrl}"${editAttr(editable, "wordmark")} style="font-family:${serifStack};font-size:${wordmarkSize}px;font-style:italic;font-weight:700;color:${fg};text-decoration:none;letter-spacing:0.005em;line-height:1.2;">${wordmark}</a>`
    : `<span${editAttr(editable, "wordmark")} style="font-family:${serifStack};font-size:${wordmarkSize}px;font-style:italic;font-weight:700;color:${fg};letter-spacing:0.005em;line-height:1.2;">${wordmark}</span>`;
...
const schema = {
  ...
  fields: [
    ...
    { key: "wordmark_size", label: "Wordmark text size", type: "fontSize", optional: true },
