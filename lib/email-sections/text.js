...
const { escHtml, renderBody, scaleFont, editAttr, richAttr, richInline } = require("./_helpers");
...
function render(props = {}, brand, editable) {
  ...
  let inner = "";
  if (headline) {
    inner += `<div${editAttr(editable, "headline")} style="font-family:${FONTS.heading};font-size:${headlineSize}px;font-weight:700;color:${BRAND.ink};line-height:${Math.round(headlineSize * (30 / 22))}px;text-align:${align};margin:0 0 8px 0;">${headline}</div>`;
  }
  if (subhead) {
  ...
  if (bodyHtml) {
    inner += `<div${editAttr(editable, "body")}${richAttr(editable)} style="font-family:${FONTS.body};font-size:${bodySize}px;color:${BRAND.inkSoft};line-height:${Math.round(bodySize * (26 / 16))}px;text-align:${align};margin:0;">${bodyHtml}</div>`;
  }
