...
<div${headlineClass}${editAttr(editable, "headline")} style="font-family:${FONTS.heading};font-size:${headlineSize}px;font-weight:700;color:${BRAND.ink};line-height:${Math.round(headlineSize * (40 / 32))}px;margin:0;">${headline}</div>`;

  if (subhead) {
    html += `
<div${subheadClass}${editAttr(editable, "subhead")} style="font-family:${FONTS.body};font-size:${subheadSize}px;color:${BRAND.inkSoft};line-height:${Math.round(subheadSize * (26 / 17))}px;margin:12px 0 0 0;">${subhead}</div>`;
  }
  html += `
</td></tr>`;
