...
function render(props = {}, brand) {
  ...
  // Social links: section's socialLinks wins ONLY if it has at least
  // one item with a real URL — otherwise fall back to brand-kit socials.
  // Prevents empty starter rows in the array editor from blocking the
  // My Settings fallback.
  const propsSocialsValid = Array.isArray(props.socialLinks)
    ? props.socialLinks.filter(s => s && typeof s.url === "string" && s.url.trim())
    : [];
  const socialLinksRaw = propsSocialsValid.length > 0 ? propsSocialsValid : brandSocials;
