...
const { escHtml, fallback, safeUrl, scaleFont, safeHex, editAttr, richAttr, richInline } = require("./_helpers");
...
// Icons render as hosted outline PNGs (email-safe — inline SVG is stripped
// by Gmail/Outlook/Yahoo). Each stock icon is pre-rendered in every tile
// theme colour at /public/email-icons/<icon>-<theme>.png. See
// scripts note: regenerate with the headless-Chrome generator if the icon
// set or theme colours change.
const ICON_BASE = "https://marketing.travelify.io/email-icons";
const DEFAULT_ICON = "sparkles";

// Stock icon library — stroke-based outline icons. Kept for the schema's
// icon picker; rendered as the matching emoji (STOCK_EMOJI) at output time
...
const STOCK_ICONS = {
  ...
  clock: '<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"/><polyline points="12 7 12 12 15.5 14"/></svg>',
  map: '<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="1 6 8 3 16 6 23 3 23 18 16 21 8 18 1 21 1 6"/><line x1="8" y1="3" x2="8" y2="18"/><line x1="16" y1="6" x2="16" y2="21"/></svg>',
...
function renderFeatureRow(feature, isLast, brand, idx, editable) {
  ...
  const text = feature.text ? richInline(feature.text) : "";
  if (!heading && !text) return "";

  // Tile colours: theme by name, with optional per-row hex overrides.
  const themeKey = TILE_THEMES[String(feature.colour || "").toLowerCase()]
    ? String(feature.colour).toLowerCase()
    : DEFAULT_THEME;
  const theme = TILE_THEMES[themeKey];
  const tileBg = safeHex(feature.tile_colour) || theme.bg;

  // Icon: hosted outline PNG, pre-coloured to the tile theme (email-safe).
  // Priority: a custom image URL, then a custom emoji, then the stock icon
  // PNG for the chosen icon + theme.
  const iconKey = STOCK_ICONS[String(feature.icon || "").toLowerCase()]
    ? String(feature.icon).toLowerCase()
    : DEFAULT_ICON;
  const customUrl = safeUrl(feature.icon_url);
  const customEmoji = feature.icon_emoji && String(feature.icon_emoji).trim();
  let glyphCell;
  if (customUrl) {
    glyphCell = `<img src="${customUrl}" width="24" height="24" alt="" style="display:inline-block;width:24px;height:24px;border:0;vertical-align:middle;">`;
  } else if (customEmoji) {
    glyphCell = `<span style="font-size:24px;line-height:48px;">${escHtml(customEmoji)}</span>`;
  } else {
    glyphCell = `<img src="${ICON_BASE}/${iconKey}-${themeKey}.png" width="24" height="24" alt="" style="display:inline-block;width:24px;height:24px;border:0;vertical-align:middle;">`;
  }
      ...
      <tr>
        <td width="66" valign="top" style="padding-right:18px;">
          <table role="presentation" border="0" cellspacing="0" cellpadding="0" width="48">
            <tr>
              <td bgcolor="${tileBg}" align="center" valign="middle" width="48" height="48" style="background-color:${tileBg};border-radius:12px;width:48px;height:48px;text-align:center;line-height:48px;mso-line-height-rule:exactly;">
                ${glyphCell}
              </td>
            </tr>
          </table>
        ...
        <td valign="top">
          ...
          ${text ? `<div${editAttr(editable, `features.${idx}.text`)}${richAttr(editable)} style="font-family:${FONTS.body};font-size:15px;line-height:1.55;color:${DESIGN_TOKENS.textSecondary};margin:0;">${text}</div>` : ""}
        </td>
      </tr>
    </table>
  </td>
  ...
  const headerHtml = `
    ...
    <tr>
      <td align="left" style="padding:0 0 18px 0;">
        <p${editAttr(editable, "lede")}${richAttr(editable)} style="font-family:${FONTS.body};font-size:${ledeSize}px;line-height:1.6;color:${DESIGN_TOKENS.textSecondary};margin:0;max-width:480px;">${lede}</p>
      </td>
    </tr>` : ""}`;
...
const schema = {
  ...
  fields: [
    ...
    { key: "features", label: "Features", type: "array", required: true, itemLabel: "feature", itemFields: [
      ...
      { key: "icon_url", label: "Custom icon image URL (overrides icon)", type: "url", optional: true, help: "Link to a small square PNG/GIF to use instead of the stock icon." },
      { key: "icon_emoji", label: "Custom emoji (overrides icon)", type: "text", optional: true, maxLength: 8, help: "Or paste an emoji to use instead of the icon." },
...
module.exports = { render, schema, STOCK_ICONS, TILE_THEMES, ICON_BASE };
