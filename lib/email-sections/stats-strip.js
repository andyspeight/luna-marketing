...
function renderStat(stat, brand, editable, index) {
  ...
  const valueSize = scaleFont(36, stat.value_size);

  return `
<div style="text-align:center;">
  <div${editAttr(editable, `stats.${index}.value`)} style="font-family:${FONTS.heading};font-size:${valueSize}px;font-weight:800;letter-spacing:-0.035em;line-height:1;color:${DESIGN_TOKENS.primary};margin:0 0 8px 0;font-variant-numeric:tabular-nums;">${value || "&nbsp;"}</div>
  <div${editAttr(editable, `stats.${index}.label`)} style="font-family:${FONTS.body};font-size:12px;font-weight:500;color:${DESIGN_TOKENS.textSecondary};line-height:1.4;margin:0;">${label}</div>
</div>`.trim();
}
...
const schema = {
  ...
  fields: [
    { key: "stats", label: "Stats (exactly 3)", type: "array", required: true, max: 3, itemLabel: "stat", itemFields: [
      ...
      { key: "value_size", label: "Number text size", type: "fontSize", optional: true },
