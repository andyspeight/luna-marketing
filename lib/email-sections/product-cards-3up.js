...
function render(props = {}, brand, editable) {
  ...
  // Keep each card's original index (its position in props.cards) so the
  // on-canvas edit propPath stays aligned with the source array even after
  // slicing/filtering empty entries.
  const cards = cardsRaw
    .map((c, i) => ({ card: c, idx: i }))
    .slice(0, 3)
    .filter(({ card }) => card && (card.image_url || card.title));
  ...
  // Render a single card cell. `idx` is the card's original index in
  // props.cards (for the on-canvas edit propPath).
  const renderCard = ({ card, idx }) => {
    ...
    // Per-card text sizes — bases match the px values in the markup below.
    const titleSize = scaleFont(14, card.title_size);
    const bodySize = scaleFont(12, card.body_size);
    ...
    const titleHtml = title
      ? `<p${editAttr(editable, `cards.${idx}.title`)} style="font-family:${FONTS.heading};font-size:${titleSize}px;font-weight:600;color:${textColour};line-height:1.3;margin:14px 0 0 0;text-align:center;">${title}</p>`
      : "";

    const bodyHtml = body
      ? `<p${editAttr(editable, `cards.${idx}.body`)} style="font-family:${FONTS.body};font-size:${bodySize}px;color:${textColour};line-height:1.5;margin:6px 0 0 0;text-align:center;opacity:0.75;">${body}</p>`
      : "";
