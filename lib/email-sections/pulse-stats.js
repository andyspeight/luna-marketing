...
<tr><td bgcolor="${DESIGN_TOKENS.primary}" class="tg-pad-mobile" style="background-color:${DESIGN_TOKENS.primary};padding:36px;color:#ffffff;">
  <div style="font-family:${FONTS.body};font-size:11px;font-weight:700;letter-spacing:0.12em;text-transform:uppercase;color:${DESIGN_TOKENS.accentLight};line-height:14px;margin:0 0 6px 0;">${label}</div>
  ${title ? `<div style="font-family:${FONTS.heading};font-size:22px;font-weight:700;letter-spacing:-0.02em;color:#ffffff;line-height:1.2;margin:0 0 24px 0;">${title}</div>` : `<div style="height:24px;line-height:24px;font-size:24px;">&nbsp;</div>`}
  ${statsHtml}
  ${source ? `
