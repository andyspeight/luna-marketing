// lib/email-sections/masthead.js
// Archetype A (B2B Weekly): newspaper-style header.
// White background, 3px navy bottom border (Outlook-safe — no
// pseudo-elements). Vol/issue and date split across a top meta row,
// large bold title, italic tagline below.
//
// See travelgenix-email-design SKILL.md §4 and Appendix A.1.
//
// Outlook patterns used:
//   - P6 (two-column without flex/grid) for the vol-issue/date row

const STATIC_BRAND = require("../email-brand");
const { escHtml, fallback, scaleFont, editAttr, safeHex } = require("./_helpers");

function render(props = {}, brand, editable) {
  const { DESIGN_TOKENS, FONTS } = brand || STATIC_BRAND;

  const issueLabel = escHtml(fallback(props.issue_label, ""));
  const dateLabel = escHtml(fallback(props.date_label, ""));
  const title = escHtml(fallback(props.title, "The Travelgenix Weekly"));
  const tagline = props.tagline ? escHtml(props.tagline) : "";

  const headingColour = safeHex(props.heading_colour) || DESIGN_TOKENS.primary;
  const bodyColour = safeHex(props.body_colour) || DESIGN_TOKENS.textSecondary;
  const eyebrowColour = safeHex(props.eyebrow_colour) || DESIGN_TOKENS.textSecondary;

  // Per-text-area sizes. Bases match the px values used in the markup below.
  const titleSize = scaleFont(38, props.title_size);
  const taglineSize = scaleFont(12, props.tagline_size);

  // Top meta row (vol/issue left, date right) — only render the row if
  // either is set. If only one is set, push it to the relevant edge.
  let metaRowHtml = "";
  if (issueLabel || dateLabel) {
    metaRowHtml = `
    <tr>
      <td style="padding:0 0 16px 0;">
        <table role="presentation" border="0" cellspacing="0" cellpadding="0" width="100%">
          <tr>
            <td${editAttr(editable, "issue_label")} valign="middle" align="left" style="font-family:${FONTS.body};font-size:11px;font-weight:500;letter-spacing:0.08em;text-transform:uppercase;color:${eyebrowColour};line-height:16px;">${issueLabel}</td>
            <td${editAttr(editable, "date_label")} valign="middle" align="right" style="font-family:${FONTS.body};font-size:11px;font-weight:500;letter-spacing:0.08em;text-transform:uppercase;color:${eyebrowColour};line-height:16px;">${dateLabel}</td>
          </tr>
        </table>
      </td>
    </tr>`;
  }

  // Title + tagline. The 3px bottom border lives on the outer cell so
  // Outlook honours it (a div border-bottom inside doesn't always render).
  return `
<tr><td bgcolor="${DESIGN_TOKENS.bgPrimary}" class="tg-pad-mobile" style="background-color:${DESIGN_TOKENS.bgPrimary};padding:32px 36px 20px;border-bottom:3px solid ${DESIGN_TOKENS.primary};">
  <table role="presentation" border="0" cellspacing="0" cellpadding="0" width="100%">
    ${metaRowHtml}
    <tr>
      <td style="padding:0;">
        <h1${editAttr(editable, "title")} style="font-family:${FONTS.heading};font-size:${titleSize}px;font-weight:800;letter-spacing:-0.035em;line-height:1;color:${headingColour};margin:0;mso-line-height-rule:exactly;">${title}</h1>
      </td>
    </tr>
    ${tagline ? `
    <tr>
      <td style="padding:8px 0 0 0;">
        <p${editAttr(editable, "tagline")} style="font-family:${FONTS.body};font-size:${taglineSize}px;font-style:italic;letter-spacing:-0.005em;color:${bodyColour};line-height:${Math.round(taglineSize * (18 / 12))}px;margin:0;">${tagline}</p>
      </td>
    </tr>` : ""}
  </table>
</td></tr>`;
}

const schema = {
  type: "masthead",
  label: "Masthead",
  description: "Newspaper-style header with vol/issue line, large bold title, and italic tagline. For B2B Weekly newsletters.",
  fields: [
    { key: "issue_label", label: "Issue label (e.g. \"Vol. 02 · Issue 19\")", type: "text", optional: true, maxLength: 60 },
    { key: "date_label", label: "Date (e.g. \"Friday, 8 May 2026\")", type: "text", optional: true, maxLength: 40 },
    { key: "eyebrow_colour", label: "Issue / date colour", type: "colour", optional: true, defaultPreview: "#475569", help: "Leave empty to keep the default." },
    { key: "title", label: "Title", type: "text", required: true, maxLength: 60 },
    { key: "title_size", label: "Title text size", type: "fontSize", optional: true },
    { key: "heading_colour", label: "Title colour", type: "colour", optional: true, defaultPreview: "#1B2B5B", help: "Leave empty to keep the default." },
    { key: "tagline", label: "Tagline (italic, single line)", type: "text", optional: true, maxLength: 120 },
    { key: "tagline_size", label: "Tagline text size", type: "fontSize", optional: true },
    { key: "body_colour", label: "Tagline colour", type: "colour", optional: true, defaultPreview: "#475569", help: "Leave empty to keep the default." },
  ],
};

module.exports = { render, schema };
