...
// Email-safe glyphs. Inline SVG (used previously) is stripped by Gmail,
// Outlook, Yahoo and most webmail — it only renders in Apple Mail — so the
// tiles showed up empty in real inboxes. Emoji render everywhere, so each
// stock icon name maps to a matching emoji drawn inside the coloured tile.
const STOCK_EMOJI = {
  sparkles: "✨", star: "⭐", mail: "✉️", file: "📄",
  pin: "📍", chat: "💬", phone: "📞", calendar: "📅",
  globe: "🌍", plane: "✈️", search: "🔍", tag: "🏷️",
  heart: "❤️", shield: "🛡️", bolt: "⚡", users: "👥",
  check: "✅", clock: "🕒", map: "🗺️", camera: "📷",
};
...
function renderFeatureRow(feature, isLast, brand, idx, editable) {
  ...
  // Icon glyph (email-safe emoji): a custom emoji wins, else the stock
  // icon name maps to its emoji, else a sensible default.
  const iconKey = feature.icon ? String(feature.icon).toLowerCase() : "";
  const glyph = escHtml(
    (feature.icon_emoji && String(feature.icon_emoji).trim()) ||
    STOCK_EMOJI[iconKey] ||
    DEFAULT_EMOJI
  );
      ...
      <tr>
        <td width="66" valign="top" style="padding-right:18px;">
          <table role="presentation" border="0" cellspacing="0" cellpadding="0" width="48">
            <tr>
              <td bgcolor="${tileBg}" align="center" valign="middle" width="48" height="48" style="background-color:${tileBg};border-radius:12px;width:48px;height:48px;text-align:center;mso-line-height-rule:exactly;">
                <span style="font-size:24px;line-height:48px;">${glyph}</span>
              </td>
            </tr>
          </table>
        </td>
...
const schema = {
  ...
  fields: [
    ...
    { key: "features", label: "Features", type: "array", required: true, itemLabel: "feature", itemFields: [
      ...
      { key: "icon_emoji", label: "Custom emoji (overrides icon)", type: "text", optional: true, maxLength: 8, help: "Paste any emoji to use instead of the chosen icon." },
