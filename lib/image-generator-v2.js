...
// Slot dimensions are chosen so that the resulting PNG (at device scale 2x)
// is safe on every social platform we publish to via Metricool. The single
// biggest constraint is Instagram, which rejects anything outside 1.91:1
// (landscape) to 4:5 (portrait). The "hero" landscape slot is set to
// 1200x630 — the established Open Graph / Facebook link-preview standard.
// 1200/630 = 1.905:1, which sits SAFELY under Instagram's 1.91:1 ceiling
// (1200/628 would be 1.9108:1 — fractionally over and IG rejects it).
//
// History: until 21 May 2026 "hero" was 1200x600 (2:1) which Instagram
// rejected because 2:1 is taller than its 1.91:1 landscape ceiling.
// "hero-tall" was 1200x800 (1.5:1, still landscape), then 1200x1500 —
// which is 4:5, NOT the 2:3 every consumer of this slot assumes. Displayed
// in a 2:3 Pinterest frame, a 4:5 image gets ~17% of its width cropped off
// (headlines lost their first letters). Fixed 2 July 2026 to 1000x1500,
// a true 2:3.
const SLOT_DIMENSIONS = {
  "hero":            { width: 1200, height: 630 },   // 1.905:1 — FB / LI / IG-safe landscape (Open Graph standard)
  "hero-square":     { width: 1200, height: 1200 },  // 1:1     — universal safe
  "hero-tall":       { width: 1000, height: 1500 },  // 2:3     — Pinterest / portrait (true 2:3)
  "hero-wide":       { width: 1200, height: 675 },   // 16:9    — X / Twitter
...
async function generateImage(input) {
  ...
  // use it.
  // Social images must be LIGHT. This lives here (not only in the brief
  // generator) because callers like the post Composer pass briefOverride,
  // which skips brief generation entirely — a rule that only exists there
  // never reaches those renders.
  const socialLightRule = input.outputContext === "social"
    ? `

BACKGROUND RULE FOR SOCIAL OUTPUT (hard requirement): the page background MUST be light — white, #F8FAFC, or a very soft teal-tinted paper. Never a dark or navy canvas, no matter what the brief's mood suggests (night-themed copy included: express night with navy TYPE, a clock or moon motif, on a light ground). Navy #1B2B5B is for text and small solid elements; teal #00B4D8 for accents. If the product itself is dark (e.g. the Luna Chat widget), render it faithfully dark ON the light background — that contrast is the point.`
    : "";

  const userMessageText = screenshotImage
    ? `Brief: ${brief}

Canvas: ${width}x${height}px (this is the design size, the renderer outputs at ${DEFAULT_DEVICE_SCALE}x for retina)${socialLightRule}
