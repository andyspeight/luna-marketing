...
const { escHtml, renderBody, scaleFont, editAttr, richAttr, richInline, safeHex } = require("./_helpers");
...
function render(props = {}, brand, editable) {
  ...
  // User-overridable text colours. Fall back to the existing brand colours
  // so emails without these props render byte-identically.
  const headingColour = safeHex(props.heading_colour) || BRAND.ink;
  const bodyColour = safeHex(props.body_colour) || BRAND.inkSoft;

  let inner = "";
  if (headline) {
    inner += `<div${editAttr(editable, "headline")} style="font-family:${FONTS.heading};font-size:${headlineSize}px;font-weight:700;color:${headingColour};line-height:${Math.round(headlineSize * (30 / 22))}px;text-align:${align};margin:0 0 8px 0;">${headline}</div>`;
  }
  if (subhead) {
    inner += `<div${editAttr(editable, "subhead")}${richAttr(editable)} style="font-family:${FONTS.body};font-size:${subheadSize}px;color:${BRAND.inkMuted};line-height:${Math.round(subheadSize * (22 / 15))}px;text-align:${align};margin:0 0 12px 0;">${subhead}</div>`;
  }
  if (bodyHtml) {
    inner += `<div${editAttr(editable, "body")}${richAttr(editable)} style="font-family:${FONTS.body};font-size:${bodySize}px;color:${bodyColour};line-height:${Math.round(bodySize * (26 / 16))}px;text-align:${align};margin:0;">${bodyHtml}</div>`;
  }
...
const schema = {
  ...
  fields: [
    ...
    { key: "body_colour", label: "Text colour", type: "colour", optional: true, defaultPreview: "#334155", help: "Leave empty to keep the default." },
