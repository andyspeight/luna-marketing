...
function render(props = {}, brand, editable) {
  ...
  const attributionHtml = (name || role)
    ? `<div style="font-family:${FONTS.body};font-size:13px;color:${bodyColour};line-height:20px;font-style:normal;margin:0;">
         ${name ? `<span${editAttr(editable, "attribution.name")} style="font-weight:600;color:${headingColour};">${name}</span>` : ""}
         ${name && role ? ` &middot; ` : ""}
         ${role}
       </div>`
    : "";
...
<tr><td bgcolor="${DESIGN_TOKENS.bgSecondary}" class="tg-pad-mobile" style="background-color:${DESIGN_TOKENS.bgSecondary};padding:36px 32px;border-bottom:1px solid ${DESIGN_TOKENS.border};">
  ${label ? `<div${editAttr(editable, "label")} style="font-family:${FONTS.body};font-size:${labelSize}px;font-weight:700;letter-spacing:0.12em;text-transform:uppercase;color:${eyebrowColour};margin:0 0 14px 0;line-height:${Math.round(labelSize * (14 / 10))}px;">${label}</div>` : ""}
  <div${editAttr(editable, "quote")} style="font-family:${FONTS.heading};font-size:${quoteSize}px;font-weight:500;line-height:${Math.round(quoteSize * (29 / 20))}px;letter-spacing:-0.015em;color:${headingColour};margin:0 0 16px 0;font-style:italic;">
  ...
  <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%">
    <tr>
      ...
      <td style="padding:0 0 0 28px;">
        <div${editAttr(editable, "quote")} style="font-family:${FONTS.heading};font-size:${quoteSize}px;font-weight:500;line-height:${Math.round(quoteSize * (28 / 19))}px;letter-spacing:-0.015em;color:${headingColour};margin:0 0 16px 0;font-style:italic;">${quote}</div>
        ${attributionHtml}
      </td>
    </tr>
  </table>
...
const schema = {
  ...
  fields: [
    ...
    { key: "eyebrow_colour", label: "Eyebrow label colour", type: "colour", optional: true, defaultPreview: "#00B4D8", help: "Leave empty to keep the default." },
    ...
    { key: "heading_colour", label: "Quote colour", type: "colour", optional: true, defaultPreview: "#0F172A", help: "Leave empty to keep the default." },
    { key: "body_colour", label: "Attribution colour", type: "colour", optional: true, defaultPreview: "#475569", help: "Leave empty to keep the default." },
