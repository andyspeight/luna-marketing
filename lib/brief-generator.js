...
S5. LIGHT backgrounds, always. Social feeds are bright white/light-grey environments; a dark navy canvas reads as a murky hole in the feed and its detail is hard to see at phone size. The canvas is a light surface — white, #F8FAFC, or a soft paper-to-teal-tint gradient — with navy (#1B2B5B) for text and teal (#00B4D8) for accents. This is a hard rule with NO mood exception: night-themed or dramatic copy is expressed through navy type, a clock or moon motif, bold scale — never a dark page. A dark PRODUCT (the Luna Chat widget is dark) renders faithfully dark ON the light canvas; that contrast is the look.
...
P8. PRODUCT-CENTRED COMPOSITION FOR EMAIL OUTPUT.
    ...
    Compose as follows: the product (Luna Chat widget, whichever product the spec describes) rendered at its full natural proportions, sized so it occupies between 60% and 75% of the canvas height. Centred horizontally on the canvas. The background is a clean brand-tinted surface. For EMAIL output the default is a soft navy gradient running diagonally from #1B2B5B at one corner to #0F1F4E at the opposite corner (a flat #0F1F4E or a subtle teal-to-navy gradient also work). For SOCIAL output, rule S5 applies with no exception: a LIGHT studio surface (white, #F8FAFC, or a soft paper-to-teal tint) with the product's own navy providing the contrast — never a dark canvas. Choose ONE clean background, no overlays, no patterns, no textures.
