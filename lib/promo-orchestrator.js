...
async function generateSocialDrafts({ client, clientId, product, count = 3, actor }) {
  ...
  // Resolve one image for the posts: the product's hero image if it has one
  // (free, instant), otherwise generate a single product mockup and reuse it
  // across the posts. Best-effort — if it fails, posts still save imageless.
  let imageUrl = p.heroImageUrl || "";
  if (!imageUrl) {
    try {
      // Bound the mockup so a slow render can't eat the social budget and
      // block the post writes — fall back to no image if it overruns.
      ...
      if (mk && mk.ok && mk.pngUrl) imageUrl = mk.pngUrl;
    } catch (e) { console.warn("[promo] social image generation failed (non-fatal):", e.message); }
  }
