...
const COMPOSER_TEMPERATURE = 0.7;

// Refinement gets a higher temperature for more variation
const REFINEMENT_TEMPERATURE = 0.8;
...
const LAYER_2_HARD_RULES = `# THE FIVE NON-NEGOTIABLE LAWS
...
- Em dashes (—). Use commas, full stops, or restructured sentences.
- Oxford commas. "X, Y and Z" never "X, Y, and Z".
- These words/phrases: leverage, utilise, synergy, game-changer, innovative, cutting-edge, delve, seamlessly, robust, best-in-class, world-class, next-generation, revolutionary, supercharge, "in today's digital landscape", "navigate the complexities", "unlock the potential", "elevate your business", "transform your".
- Defensive italicised emphasis ("I *genuinely*", "what I *can* do is").
...
function runCoreValidation(text) {
  ...
  // Em dash — HARD BAN
  if (/—/.test(text)) {
    hardErrors.push("Em dash detected — composer output rejected. Re-prompt required.");
  }
...
async function compose(input, adaptor) {
  ...
  // 5. Validate
  const coreValidation = runCoreValidation(JSON.stringify(draft));
