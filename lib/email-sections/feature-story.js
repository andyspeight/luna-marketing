...
const { escHtml, safeUrl, fallback, scaleFont, editAttr, safeHex } = require("./_helpers");
...
function render(props = {}, brand, editable) {
  ...
  // so emails without these props render byte-identically. heading_colour
  // covers the section title and the in-card article title; body_colour
  // covers the lede and the excerpt.
...
<tr><td bgcolor="${DESIGN_TOKENS.bgPrimary}" class="tg-pad-mobile" style="background-color:${DESIGN_TOKENS.bgPrimary};padding:56px 32px 16px;">
  <div${editAttr(editable, "eyebrow")} style="font-family:${FONTS.body};font-size:${eyebrowSize}px;font-weight:600;letter-spacing:0.1em;text-transform:uppercase;color:${eyebrowColour};margin:0 0 12px 0;line-height:${Math.round(eyebrowSize * (18 / 11))}px;">${eyebrow}</div>
  <div${editAttr(editable, "title")} style="font-family:${FONTS.heading};font-size:${titleSize}px;font-weight:700;line-height:${Math.round(titleSize * (32 / 26))}px;letter-spacing:-0.02em;color:${headingColour};margin:0 0 8px 0;">${title}</div>
  ${lede ? `<div${editAttr(editable, "lede")} style="font-family:${FONTS.body};font-size:${ledeSize}px;line-height:${Math.round(ledeSize * (24 / 15))}px;color:${bodyColour};margin:0 0 32px 0;max-width:520px;">${lede}</div>` : `<div style="height:32px;line-height:32px;font-size:32px;">&nbsp;</div>`}
</td></tr>
  ...
  <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" bgcolor="${DESIGN_TOKENS.bgSecondary}" style="background-color:${DESIGN_TOKENS.bgSecondary};border:1px solid ${DESIGN_TOKENS.border};border-radius:16px;overflow:hidden;width:100%;">
    ...
    <tr>
      <td style="padding:32px;">
        <div${editAttr(editable, "body_title")} style="font-family:${FONTS.heading};font-size:${bodyTitleSize}px;font-weight:700;line-height:${Math.round(bodyTitleSize * (28 / 22))}px;letter-spacing:-0.02em;color:${headingColour};margin:0 0 12px 0;">${bodyTitle}</div>
        ${excerpt ? `<div${editAttr(editable, "excerpt")} style="font-family:${FONTS.body};font-size:${excerptSize}px;line-height:${Math.round(excerptSize * (25 / 15))}px;color:${bodyColour};margin:0 0 20px 0;">${excerpt}</div>` : ""}
        ${linkHtml}
      </td>
    </tr>
  </table>
...
const schema = {
  ...
  fields: [
    ...
    { key: "heading_colour", label: "Heading colour", type: "colour", optional: true, defaultPreview: "#0F172A", help: "Leave empty to keep the default. Applies to the section title and the article title." },
    ...
    { key: "body_colour", label: "Text colour", type: "colour", optional: true, defaultPreview: "#475569", help: "Leave empty to keep the default. Applies to the lede and the article excerpt." },
