...
<div style="font-family:${FONTS.heading};font-size:18px;font-weight:700;line-height:1.3;letter-spacing:-0.015em;color:${DESIGN_TOKENS.textPrimary};margin:0 0 8px 0;">${headline}</div>
${body ? `<p style="font-family:${FONTS.body};font-size:14px;line-height:1.6;color:${DESIGN_TOKENS.textSecondary};margin:0 0 12px 0;">${body}</p>` : ""}
${linkUrl ? `<a href="${linkUrl}" style="font-family:${FONTS.body};font-size:13px;font-weight:600;color:${DESIGN_TOKENS.primary};text-decoration:none;display:inline-block;line-height:18px;">${linkText}</a>` : ""}`.trim();
}

function render(props = {}) {
  const left = renderItem(props.left);
  const right = renderItem(props.right);
