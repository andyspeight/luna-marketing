...
function render(props = {}, brand, editable) {
  ...
  // Text colour overrides. Default keeps the variant-computed colour so
  // existing emails are unchanged; either can be overridden with a valid hex.
  const headingColour = safeHex(props.heading_colour) || headlineColor;
  const bodyColour = safeHex(props.body_colour) || bodyColor;

  let inner = "";
  if (headline) {
    inner += `<div${editAttr(editable, "headline")} style="font-family:${FONTS.heading};font-size:${headlineSize}px;font-weight:700;color:${headingColour};line-height:${Math.round(headlineSize * (32 / 24))}px;text-align:center;margin:0 0 12px 0;">${headline}</div>`;
  }
  if (body) {
    inner += `<div${editAttr(editable, "body")}${richAttr(editable)} style="font-family:${FONTS.body};font-size:${bodySize}px;color:${bodyColour};line-height:${Math.round(bodySize * (24 / 15))}px;text-align:center;margin:0 0 24px 0;">${body}</div>`;
  } else {
...
const schema = {
  ...
  fields: [
    ...
    { key: "body_colour", label: "Text colour", type: "colour", optional: true, defaultPreview: "#334155", placeholder: "Defaults to current text colour", help: "Colour of the supporting copy. Leave empty to keep the default." },
