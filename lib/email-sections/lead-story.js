...
function renderByline(byline, brand, editable) {
  ...
  <td style="padding:14px 0;border-top:1px solid ${DESIGN_TOKENS.borderLight};border-bottom:1px solid ${DESIGN_TOKENS.borderLight};">
    <table role="presentation" border="0" cellspacing="0" cellpadding="0" width="100%">
      <tr>
        ...
        <td valign="middle">
          ${name ? `<div${editAttr(editable, "byline.name")} style="font-family:${FONTS.body};font-size:13px;font-weight:600;color:${DESIGN_TOKENS.textPrimary};line-height:18px;margin:0;">${name}</div>` : ""}
          ${meta ? `<div${editAttr(editable, "byline.meta")} style="font-family:${FONTS.body};font-size:12px;color:${DESIGN_TOKENS.textSecondary};line-height:18px;margin:0;">${meta}</div>` : ""}
        </td>
      </tr>
    </table>
  </td>
...
function render(props = {}, brand, editable) {
  ...
  // Author-written text colours. Defaults keep the original treatment
  // (headline = primary ink, body = secondary ink); either can be
  ...
  const linkHtml = linkUrl ? `
    <tr>
      <td style="padding:8px 0 0 0;">
        <a href="${linkUrl}"${editAttr(editable, "link_text")} style="display:inline-block;font-family:${FONTS.body};font-size:14px;font-weight:600;color:${ctaFg};text-decoration:none;border-bottom:2px solid ${ctaBg};padding-bottom:2px;line-height:20px;">${linkText}</a>
      </td>
    </tr>` : "";
  ...
  <table role="presentation" border="0" cellspacing="0" cellpadding="0" width="100%">
    <tr>
      <td style="padding:0 0 12px 0;">
        <span${editAttr(editable, "kicker")} style="font-family:${FONTS.body};font-size:${kickerSize}px;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;color:${DESIGN_TOKENS.accent};line-height:${Math.round(kickerSize * (14 / 11))}px;">— ${kickerText}</span>
      </td>
    </tr>
    <tr>
      <td style="padding:0 0 14px 0;">
        <h2${editAttr(editable, "headline")} style="font-family:${FONTS.heading};font-size:${headlineSize}px;font-weight:700;line-height:1.2;letter-spacing:-0.025em;color:${headingColour};margin:0;">${headline}</h2>
      </td>
    </tr>
    ${deck ? `
    <tr>
      <td style="padding:0 0 24px 0;">
        <p${editAttr(editable, "deck")} style="font-family:${FONTS.body};font-size:${deckSize}px;line-height:1.55;color:${DESIGN_TOKENS.textSecondary};font-weight:400;margin:0;letter-spacing:-0.005em;">${deck}</p>
      </td>
    </tr>` : ""}
    ${imageHtml}
    ${renderByline(byline, brand, editable)}
    ${body ? `
    <tr>
      <td style="padding:24px 0 0 0;">
        <div${editAttr(editable, "body")}${richAttr(editable)} style="font-family:${FONTS.body};font-size:${bodySize}px;line-height:1.7;color:${bodyColour};">${body}</div>
      </td>
    </tr>` : ""}
    ${linkHtml}
  </table>
...
const schema = {
  ...
  fields: [
    ...
    { key: "heading_colour", label: "Headline colour", type: "colour", optional: true, defaultPreview: "#0F172A", placeholder: "Defaults to current headline colour", help: "Colour of the headline text. Leave empty to keep the default." },
    ...
    { key: "body_colour", label: "Body colour", type: "colour", optional: true, defaultPreview: "#475569", placeholder: "Defaults to current body colour", help: "Colour of the article body text. Leave empty to keep the default." },
