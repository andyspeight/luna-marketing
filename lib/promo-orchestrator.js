...
async function generateSocialDrafts({ client, clientId, product, count = 3, actor }) {
  ...
  // Mockups are padded onto a branded card (nothing cropped); hero photos are
  // cropped to fill. Falls back to the single image in all buckets on failure.
  let variants = null;
  if (imageUrl) {
    variants = await buildBrandedVariants(imageUrl, {
      tenantId: clientId,
      mode: imageSource === "hero" ? "cover" : "pad"
    });
  }
