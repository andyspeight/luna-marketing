...
function render(props = {}) {
  ...
  let inner = "";
  if (headline) {
    inner += `<div style="font-family:${FONTS.heading};font-size:${headlineSize}px;font-weight:700;color:${headlineColor};line-height:${Math.round(headlineSize * (32 / 24))}px;text-align:center;margin:0 0 12px 0;">${headline}</div>`;
  }
  if (body) {
    inner += `<div style="font-family:${FONTS.body};font-size:${bodySize}px;color:${bodyColor};line-height:${Math.round(bodySize * (24 / 15))}px;text-align:center;margin:0 0 24px 0;">${body}</div>`;
  } else {
