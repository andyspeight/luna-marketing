...
// Colour-aware sections (May 2026) — sister to existing masthead and
// feature-story but with overridable background colour + text colour
// for marketing-newsletter aesthetics (Writesonic, The Hustle, Morning
// Brew style). All hex props are strictly validated against #RRGGBB
// in the section file before render to prevent CSS injection.
const heroColoured = require("./hero-coloured");
const storyNumbered = require("./story-numbered");
const featureTiles = require("./feature-tiles");

// Daily-Beacon-style sections (May 2026) — editorial publication feel
// with two-tone serif headlines, duo images, product cards, pill CTAs,
// full-bleed imagery, circle-icon footers. Built for Travelgenix's own
// marketing emails (product launches, feature announcements). All
// colour props strictly hex-validated.
const mastheadBar = require("./masthead-bar");
const headlineDuotone = require("./headline-duotone");
const imageDuo = require("./image-duo");
const productCards3up = require("./product-cards-3up");
const ctaPill = require("./cta-pill");
const imageFullBleed = require("./image-full-bleed");
const footerIcons = require("./footer-icons");

// Single inline image (May 2026) — everyday image section for editorial
// layout. Slots between text blocks with standard email padding. Sister
// to image-full-bleed (edge-to-edge, no padding) and image-duo (two-up).
...
// Layout + content blocks (September 2026). Fill the everyday gaps between
// the archetype-specific sections: an image beside text (left or right),
// a standalone button, a spacer, a tinted callout, a plain bullet / tick
// list and a personal sign-off. All use DESIGN_TOKENS so client brand kits
// theme them, and all emit data-tg-edit hooks for the on-canvas builder.
const imageText = require("./image-text");
const button = require("./button");
const spacer = require("./spacer");
const callout = require("./callout");
const bulletList = require("./bullet-list");
const signature = require("./signature");

// B2B SaaS blocks (September 2026) — the beats a software company's emails
// need that a travel newsletter doesn't: pricing cards, a "trusted by" logo
// strip, a tagged what's-new list, a customer-story card, before / after
// columns and an FAQ. All fact-heavy, so every schema tells the composer
// to use only Product Library / brief data.
const pricingCards = require("./pricing-cards");
const logoStrip = require("./logo-strip");
const updateList = require("./update-list");
const customerStory = require("./customer-story");
const compareColumns = require("./compare-columns");
...
const SECTIONS = {
  ...
  // Colour-aware sections — work with any archetype, designed for
  // marketing-newsletter feel with overridable bg/text/accent colours.
  "hero-coloured": heroColoured,
  "story-numbered": storyNumbered,
  "feature-tiles": featureTiles,

  // Daily-Beacon-style sections — editorial publication feel for
  // Travelgenix's own marketing/product launch emails.
  "masthead-bar": mastheadBar,
  "headline-duotone": headlineDuotone,
  "image-duo": imageDuo,
  "product-cards-3up": productCards3up,
  "cta-pill": ctaPill,
  "image-full-bleed": imageFullBleed,
  "footer-icons": footerIcons,

  // Single inline image — everyday image section, slots between text blocks
  "image": image,
  ...
  // Layout + content blocks — image beside text, button, spacer, callout,
  // bullet list, signature.
 ...
 */
function renderSection(type, props, brand, editable) {
  ...
  try {
    ...
    // `editable` (optional) asks renderers that support it to emit
    // data-tg-edit attributes for the on-canvas builder; renderers that
    // don't use it simply ignore the extra argument.
    return mod.render(props || {}, brand, editable);
  } catch (e) {
