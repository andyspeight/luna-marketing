...
async function isPromotionSlot(tenantId, channel) {
  ...
  const tenantEsc = escapeFormulaString(tenantId);
  const channelEsc = escapeFormulaString(channel);

  const formula = `AND(` +
    `FIND('${tenantEsc}', ARRAYJOIN({Client})),` +
...
async function decide(input) {
  ...
  // ---------- 3. Pull Topic Mappings for this tenant ----------
  const tenantEsc = escapeFormulaString(tenantId);
  const topicFormula = `FIND('${tenantEsc}', ARRAYJOIN({Tenant}))`;

  const topicRecords = await airtableList(TABLE_IDS.topics, {
    filterByFormula: topicFormula,
    maxRecords: 200,
    fields: ["Topic keyword", "Linked product", "Weight"]
  });
