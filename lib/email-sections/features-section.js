...
function renderFeatureRow(feature, isLast, brand) {
  ...
  <td style="padding:24px 0;${borderStyle}">
    <table role="presentation" border="0" cellspacing="0" cellpadding="0" width="100%">
      <tr>
        ...
        <td valign="top">
          ${heading ? `<div style="font-family:${FONTS.heading};font-size:18px;font-weight:700;letter-spacing:-0.015em;color:${DESIGN_TOKENS.textPrimary};line-height:1.3;margin:4px 0 6px 0;">${heading}</div>` : ""}
          ${text ? `<div style="font-family:${FONTS.body};font-size:14px;line-height:1.6;color:${DESIGN_TOKENS.textSecondary};margin:0;">${text}</div>` : ""}
        </td>
      </tr>
    </table>
  </td>
...
function render(props = {}, brand) {
  ...
  const featuresHtml = features
    .map((f, i) => renderFeatureRow(f, i === features.length - 1, brand))
    .filter(Boolean)
    .join("");
  ...
  <table role="presentation" border="0" cellspacing="0" cellpadding="0" width="100%">
    ...
    <tr>
      <td align="center" style="padding:0 0 12px 0;">
        <span style="font-family:${FONTS.body};font-size:${eyebrowSize}px;font-weight:700;letter-spacing:0.12em;text-transform:uppercase;color:${DESIGN_TOKENS.accent};line-height:${Math.round(eyebrowSize * (14 / 11))}px;">${eyebrow}</span>
      </td>
    </tr>` : ""}
    ${title ? `
    <tr>
      <td align="center" style="padding:0 0 12px 0;">
        <h2 style="font-family:${FONTS.heading};font-size:${titleSize}px;font-weight:700;line-height:1.2;letter-spacing:-0.025em;color:${DESIGN_TOKENS.textPrimary};margin:0;">${title}</h2>
      </td>
    </tr>` : ""}
    ${lede ? `
    <tr>
      <td align="center" style="padding:0 0 40px 0;">
        <p style="font-family:${FONTS.body};font-size:${ledeSize}px;line-height:1.6;color:${DESIGN_TOKENS.textSecondary};margin:0 auto;max-width:460px;text-align:center;">${lede}</p>
      </td>
