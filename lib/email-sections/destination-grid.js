...
function renderCard(card, brand, idx, editable) {
...
<table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="width:100%;border:1px solid ${DESIGN_TOKENS.border};border-radius:12px;overflow:hidden;background:${DESIGN_TOKENS.bgPrimary};">
  ...
  <tr>
    <td style="padding:20px;">
      ${cardOpen}
      ${tag ? `<div${editAttr(editable, `cards.${idx}.tag`)} style="font-family:${FONTS.body};font-size:10px;font-weight:600;letter-spacing:0.08em;text-transform:uppercase;color:${DESIGN_TOKENS.accent};line-height:14px;margin:0 0 8px 0;">${tag}</div>` : ""}
      <div${editAttr(editable, `cards.${idx}.name`)} style="font-family:${FONTS.heading};font-size:18px;font-weight:600;letter-spacing:-0.015em;color:${DESIGN_TOKENS.textPrimary};line-height:24px;margin:0 0 6px 0;">${name}</div>
      ${meta ? `<div${editAttr(editable, `cards.${idx}.meta`)} style="font-family:${FONTS.body};font-size:13px;color:${DESIGN_TOKENS.textSecondary};line-height:19px;margin:0;">${meta}</div>` : ""}
      ${priceAmount ? `
...
function render(props = {}, brand, editable) {
  ...
  const cells = cards
    .map((c, i) => `<td valign="top" width="33.33%" class="tg-stack-narrow" style="vertical-align:top;padding:0 8px;">${renderCard(c, brand, i, editable)}</td>`)
    .join("");
