...
function renderItem(item, index, isLast) {
  if (!item) return "";
...
function render(props = {}) {
  ...
  const headerHtml = `
    <table role="presentation" border="0" cellspacing="0" cellpadding="0" width="100%">
      <tr>
        <td valign="baseline" align="left" style="font-family:${FONTS.heading};font-size:20px;font-weight:700;letter-spacing:-0.02em;color:${DESIGN_TOKENS.textPrimary};line-height:1.2;">${title || "&nbsp;"}</td>
  ...
  const itemsHtml = items
    .map((item, i) => renderItem(item, i, i === items.length - 1))
    .filter(Boolean)
    .join("");
