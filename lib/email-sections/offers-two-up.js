...
function renderCard(offer) {
  ...
  const wasHtml = offer.wasFormatted
    ? `<span style="font-family:${FONTS.body};font-size:11px;color:${BRAND.inkMuted};text-decoration:line-through;margin-right:5px;">${escHtml(offer.wasFormatted)}</span>`
    : "";
...
${imageCell({ offer, height: 130 })}
</td></tr>
<tr><td style="padding:12px 12px 4px 12px;">
<div style="font-family:${FONTS.body};font-size:9px;font-weight:700;letter-spacing:0.1em;color:${BRAND.inkMuted};text-transform:uppercase;margin:0 0 3px 0;">${escHtml(locationLine(offer))}</div>
<div style="font-family:${FONTS.heading};font-size:13px;font-weight:700;color:${BRAND.ink};line-height:18px;margin:0 0 4px 0;">${escHtml(offer.name || "")}</div>
${offer.rating ? `<div style="margin:0 0 4px 0;font-size:10px;">${starRow(offer.rating)}</div>` : ""}
...
<tr><td style="padding:10px 12px 12px 12px;border-top:1px solid ${BRAND.ruleSubtle};">
<table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="width:100%;">
<tr>
<td valign="middle" style="vertical-align:middle;">
<div style="font-family:${FONTS.body};font-size:8px;color:${BRAND.inkMuted};font-weight:600;letter-spacing:0.05em;text-transform:uppercase;margin:0;">From</div>
<div style="margin-top:1px;white-space:nowrap;">
${wasHtml}<span style="font-family:${FONTS.heading};font-size:18px;font-weight:800;color:${BRAND.ink};letter-spacing:-0.02em;">${escHtml(primary)}</span><span style="font-family:${FONTS.body};font-size:10px;color:${BRAND.inkMuted};margin-left:2px;">${sub}</span>
</div>
${saving ? `<div style="font-family:${FONTS.body};font-size:10px;font-weight:700;color:${BRAND.pink};margin-top:2px;">Save &pound;${saving}pp</div>` : ""}
</td>
<td valign="middle" align="right" style="vertical-align:middle;text-align:right;">
<table role="presentation" cellspacing="0" cellpadding="0" border="0" align="right"><tr><td align="center" bgcolor="${BRAND.tealDeep}" style="background-color:${BRAND.tealDeep};border-radius:6px;">
<a href="${safe}" target="_blank" rel="noopener noreferrer" style="display:inline-block;padding:8px 12px;font-family:${FONTS.body};font-size:11px;font-weight:700;color:${BRAND.surface};text-decoration:none;border-radius:6px;">View &rarr;</a>
</td></tr></table>
</td>
</tr>
</table>
...
${heading ? `<div style="font-family:${FONTS.heading};font-size:20px;font-weight:700;color:${BRAND.ink};line-height:26px;letter-spacing:-0.01em;margin:0;">${heading}</div>` : ""}
  ...
  const cells = offers
    .map((o) => `<td valign="top" width="50%" class="tg-stack-narrow" style="vertical-align:top;padding:0 6px;">${renderCard(o)}</td>`)
    .join("");
  const filler = offers.length === 1
    ? `<td width="50%" class="tg-stack-narrow" style="display:none"></td>`
    : "";

  return `
${demoBanner(offers, { suppress: props._suppressDemoBanner })}
${headerHtml}
<tr><td bgcolor="${BRAND.surface}" class="tg-pad-mobile" style="background-color:${BRAND.surface};padding:${(eyebrow || heading) ? 8 : 20}px 20px 20px 20px;">
<table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="width:100%;">
<tr>${cells}${filler}</tr>
</table>
</td></tr>`;
