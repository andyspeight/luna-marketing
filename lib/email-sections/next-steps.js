...
const { escHtml, fallback, inlineMarkdown, scaleFont, editAttr } = require("./_helpers");

function renderStep(step, index, brand, editable) {
  ...
  <td style="padding:12px 0;">
    <table role="presentation" border="0" cellspacing="0" cellpadding="0" width="100%">
      <tr>
        ...
        <td valign="top">
          ${title ? `<div${editAttr(editable, `steps.${index}.title`)} style="font-family:${FONTS.heading};font-size:14px;font-weight:600;letter-spacing:-0.01em;color:${DESIGN_TOKENS.textPrimary};line-height:21px;margin:0 0 3px 0;">${title}</div>` : ""}
          ${text ? `<div${editAttr(editable, `steps.${index}.text`)} style="font-family:${FONTS.body};font-size:13px;line-height:20px;color:${DESIGN_TOKENS.textSecondary};margin:0;">${text}</div>` : ""}
        </td>
      </tr>
    </table>
  </td>
...
function render(props = {}, brand, editable) {
  ...
  // Block-title size. Base matches the px value in the markup below.
  const titleSize = scaleFont(16, props.title_size);

  if (!steps.length) return "";

  const stepsHtml = steps.map((d, i) => renderStep(d, i, brand, editable)).filter(Boolean).join("");
  ...
  <table role="presentation" border="0" cellspacing="0" cellpadding="0" width="100%" style="border-top:1px solid ${DESIGN_TOKENS.border};padding-top:24px;">
    <tr>
      <td style="padding:0 0 12px 0;">
        <div${editAttr(editable, "title")} style="font-family:${FONTS.heading};font-size:${titleSize}px;font-weight:700;letter-spacing:-0.015em;color:${DESIGN_TOKENS.textPrimary};line-height:${Math.round(titleSize * (22 / 16))}px;margin:0;">${title}</div>
      </td>
    </tr>
    ${stepsHtml}
  </table>
...
const schema = {
  ...
  fields: [
    ...
    { key: "title_size", label: "Block title text size", type: "fontSize", optional: true },
