...
function render(props = {}) {
  ...
  const contactsHtml = contacts.map(renderContact).filter(Boolean).join("");
...
<tr><td bgcolor="${DESIGN_TOKENS.bgSecondary}" class="tg-pad-mobile" align="center" style="background-color:${DESIGN_TOKENS.bgSecondary};padding:28px 32px;text-align:center;border-top:1px solid ${DESIGN_TOKENS.border};">
  <div style="font-family:${FONTS.heading};font-size:14px;font-weight:600;color:${DESIGN_TOKENS.textPrimary};letter-spacing:-0.01em;line-height:21px;margin:0 0 6px 0;">${title}</div>
  ${text ? `<div style="font-family:${FONTS.body};font-size:13px;line-height:20px;color:${DESIGN_TOKENS.textSecondary};margin:0 0 16px 0;">${text}</div>` : `<div style="height:16px;line-height:16px;font-size:16px;">&nbsp;</div>`}
