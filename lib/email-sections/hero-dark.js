...
function renderHeroNav(props, brand) {
...
function renderHeadline(headline, brand) {
  const { DESIGN_TOKENS, FONTS } = brand || STATIC_BRAND;
...
function render(props = {}, brand, editable) {
  ...
  const headlineHtml = renderHeadline(props.headline, brand);
  ...
  const eyebrowSize = scaleFont(12, props.eyebrow_size);
  const headlineSize = scaleFont(52, props.headline_size);
  const deckSize = scaleFont(18, props.deck_size);
  ...
  const primaryBtn = primaryCta && primaryCta.text && primaryCta.url
    ? bulletproofCta({
        ...
        bgColor: safeHex(primaryCta.colour) || DESIGN_TOKENS.accent,
        textColor: safeHex(primaryCta.text_colour) || DESIGN_TOKENS.primaryDark,
        width: 220,
        height: 50,
        align: "left",
      })
  ...
  const secondaryBtn = (() => {
    ...
    // fallback assumes solid fill. The optional `colour` override recolours
    // the border (the button's only fill), and `text_colour` the label.
    const ghostBorder = safeHex(secondaryCta.colour) || "rgba(255,255,255,0.22)";
    const ghostStroke = safeHex(secondaryCta.colour) || "#444B5C";
    const ghostText = safeHex(secondaryCta.text_colour) || "#ffffff";
    const height = 50, width = 200;
    return `
<table role="presentation" border="0" cellspacing="0" cellpadding="0" align="left" style="margin:0;">
  <tr>
    <td align="center" bgcolor="#0A0F1F" style="background-color:#0A0F1F;border:1px solid ${ghostBorder};border-radius:12px;">
      <!--[if mso]>
        <v:roundrect xmlns:v="urn:schemas-microsoft-com:vml" xmlns:w="urn:schemas-microsoft-com:office:word"
          ...
          fillcolor="#0A0F1F">
          <w:anchorlock/>
          <center style="color:${ghostText};font-family:'Segoe UI',Arial,sans-serif;font-size:15px;font-weight:600;">${text}</center>
        </v:roundrect>
      <![endif]-->
      <!--[if !mso]><!-- -->
        <a href="${url}" style="display:inline-block;padding:15px 22px;background:transparent;color:${ghostText};text-decoration:none;border-radius:12px;font-family:${FONTS.body};font-size:15px;font-weight:600;letter-spacing:-0.01em;line-height:1;">${text}</a>
      <!--<![endif]-->
    </td>
  </tr>
</table>`;
  ...
  if (primaryBtn || secondaryBtn) {
    ...
    <tr>
      <td style="padding:36px 0 0 0;">
        <table role="presentation" border="0" cellspacing="0" cellpadding="0">
          <tr>
            ${primaryBtn ? `<td valign="middle" align="left"${editAttr(editable, "primary_cta.text")} style="padding-right:12px;">${primaryBtn}</td>` : ""}
            ${secondaryBtn ? `<td valign="middle" align="left"${editAttr(editable, "secondary_cta.text")}>${secondaryBtn}</td>` : ""}
          </tr>
        </table>
      </td>
    </tr>`;
  ...
  // clients get the mesh glow as part of the td's background-image stack
  // (two radials over the flat navy). Previously the glow was a pair of
  // absolutely-positioned <div>s — Gmail strips position/filter styles,
  // which dropped ~1000px of empty navy blocks into the layout flow on
  // real sends. Background images can't break the flow.
  return `
<tr><td bgcolor="${DESIGN_TOKENS.primaryDark}" class="tg-pad-mobile" style="background-color:${DESIGN_TOKENS.primaryDark};background-image:radial-gradient(circle at -8% -12%, rgba(0,180,216,0.32) 0%, transparent 45%), radial-gradient(circle at 108% 112%, rgba(72,202,228,0.18) 0%, transparent 45%);padding:32px 40px 56px;color:#ffffff;">
  <table role="presentation" border="0" cellspacing="0" cellpadding="0" width="100%">
    ${renderHeroNav(props, brand)}
    ${eyebrow ? `
    <tr>
      <td style="padding:0 0 20px 0;">
        <span${editAttr(editable, "eyebrow")} style="font-family:${FONTS.body};font-size:${eyebrowSize}px;font-weight:600;letter-spacing:0.15em;text-transform:uppercase;color:${DESIGN_TOKENS.accentLight};line-height:${Math.round(eyebrowSize * (14 / 12))}px;">${eyebrow}</span>
      </td>
    </tr>` : ""}
    <tr>
      <td style="padding:0 0 20px 0;">
        <h1${editAttr(editable, "headline")} style="font-family:${FONTS.heading};font-size:${headlineSize}px;font-weight:800;line-height:1.02;letter-spacing:-0.04em;color:#ffffff;margin:0;mso-line-height-rule:exactly;">${headlineHtml}</h1>
      </td>
    </tr>
    ${deck ? `
    <tr>
      <td style="padding:0;">
        <p${editAttr(editable, "deck")} style="font-family:${FONTS.body};font-size:${deckSize}px;line-height:1.55;color:rgba(255,255,255,0.78);font-weight:400;letter-spacing:-0.005em;margin:0;max-width:480px;">${deck}</p>
      </td>
    </tr>` : ""}
    ${ctaHtml}
  </table>
...
const schema = {
  ...
  fields: [
    ...
    { key: "secondary_cta", label: "Secondary CTA (ghost / outline)", type: "group", optional: true, fields: [
      { key: "text", label: "Button text", type: "text", maxLength: 30 },
      { key: "url", label: "Button URL", type: "url" },
      { key: "colour", label: "Button colour", type: "colour", optional: true, defaultPreview: "#444B5C", placeholder: "Defaults to current button colour", help: "Border colour of the ghost button. Leave empty to keep the default." },
      { key: "text_colour", label: "Button text colour", type: "colour", optional: true, defaultPreview: "#FFFFFF", placeholder: "Defaults to current label colour", help: "Colour of the button label. Leave empty to keep the default." },
    ]},
  ],
};
