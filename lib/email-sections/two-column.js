...
const { escHtml, safeUrl, renderBody, fallback, scaleFont, safeHex, editAttr, richAttr } = require("./_helpers");

function render(props = {}, brand, editable) {
  const left = props.left || {};
  const right = props.right || {};

  // Text colour overrides apply to both columns' headings / body respectively.
  // Default keeps the original treatment so existing emails are unchanged;
  // either can be overridden with a valid hex.
  ...
  function renderColumnCell(col, isLeft) {
    const colKey = isLeft ? "left" : "right";
    ...
    if (headline) {
      inner += `<div${editAttr(editable, `${colKey}.headline`)} style="font-family:${FONTS.heading};font-size:${headlineSize}px;font-weight:700;color:${headingColour};line-height:${Math.round(headlineSize * (24 / 17))}px;margin:0 0 8px 0;">${headline}</div>`;
    }
    if (bodyHtml) {
      inner += `<div${editAttr(editable, `${colKey}.body`)}${richAttr(editable)} style="font-family:${FONTS.body};font-size:${bodySize}px;color:${bodyColour};line-height:${Math.round(bodySize * (22 / 14))}px;margin:0 0 8px 0;">${bodyHtml}</div>`;
    }
    if (linkText && linkUrl) {
      inner += `<div style="font-family:${FONTS.body};font-size:13px;font-weight:600;line-height:20px;margin:0;"><a href="${linkUrl}"${editAttr(editable, `${colKey}.linkText`)} style="color:${linkColour};text-decoration:none;">${linkText} &rarr;</a></div>`;
    }
...
const schema = {
  ...
  fields: [
    { key: "heading_colour", label: "Headline colour", type: "colour", optional: true, defaultPreview: "#0F172A", placeholder: "Defaults to current headline colour", help: "Colour of both columns' headlines. Leave empty to keep the default." },
    { key: "body_colour", label: "Text colour", type: "colour", optional: true, defaultPreview: "#334155", placeholder: "Defaults to current text colour", help: "Colour of both columns' body text. Leave empty to keep the default." },
