...
function render(props = {}, brand, editable) {
  ...
  function renderColumnCell(col, isLeft) {
    ...
    if (headline) {
      inner += `<div${editAttr(editable, `${colKey}.headline`)} style="font-family:${FONTS.heading};font-size:${headlineSize}px;font-weight:700;color:${BRAND.ink};line-height:${Math.round(headlineSize * (24 / 17))}px;margin:0 0 8px 0;">${headline}</div>`;
    }
    if (bodyHtml) {
      inner += `<div${editAttr(editable, `${colKey}.body`)}${richAttr(editable)} style="font-family:${FONTS.body};font-size:${bodySize}px;color:${BRAND.inkSoft};line-height:${Math.round(bodySize * (22 / 14))}px;margin:0 0 8px 0;">${bodyHtml}</div>`;
    }
    if (linkText && linkUrl) {
