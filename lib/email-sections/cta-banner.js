...
function render(props = {}, brand, editable) {
  ...
  // Text colour overrides. Default keeps the original treatment (white title,
  // translucent-white supporting text) so existing emails are unchanged;
  // either can be overridden with a valid hex.
  const headingColour = safeHex(props.heading_colour) || "#ffffff";
  const bodyColour = safeHex(props.body_colour) || "rgba(255,255,255,0.8)";
...
<tr><td bgcolor="${DESIGN_TOKENS.primary}" class="tg-pad-mobile" align="center" style="background-color:${DESIGN_TOKENS.primary};background-image:linear-gradient(135deg, ${DESIGN_TOKENS.primary} 0%, ${DESIGN_TOKENS.primaryLight} 100%);padding:48px 32px;text-align:center;color:#ffffff;">
  <div${editAttr(editable, "title")} style="font-family:${FONTS.heading};font-size:${titleSize}px;font-weight:700;letter-spacing:-0.02em;line-height:${Math.round(titleSize * (30 / 24))}px;color:${headingColour};margin:0 0 12px 0;">${title}</div>
  ${text ? `<div${editAttr(editable, "text")} style="font-family:${FONTS.body};font-size:${textSize}px;line-height:${Math.round(textSize * (24 / 15))}px;color:${bodyColour};margin:0 auto 24px auto;max-width:420px;">${text}</div>` : `<div style="height:24px;line-height:24px;font-size:24px;">&nbsp;</div>`}
  ${ctaHtml}
</td></tr>`;
}
...
const schema = {
  ...
  fields: [
    ...
    { key: "heading_colour", label: "Title colour", type: "colour", optional: true, defaultPreview: "#FFFFFF", placeholder: "Defaults to current title colour", help: "Colour of the title text. Leave empty to keep the default." },
    ...
    { key: "body_colour", label: "Text colour", type: "colour", optional: true, defaultPreview: "#E2E8F0", placeholder: "Defaults to current text colour", help: "Colour of the supporting text. Leave empty to keep the default." },
