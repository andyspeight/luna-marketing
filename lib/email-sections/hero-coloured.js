...
const { escHtml, fallback, safeUrl, editAttr, richAttr, richInline } = require("./_helpers");
    ...
    <tr>
      <td style="padding:0 0 16px 0;border-bottom:1px solid ${dividerColour};">
        <table role="presentation" border="0" cellspacing="0" cellpadding="0" width="100%">
          <tr>
            <td valign="middle" align="left"${editAttr(editable, "eyebrow")} style="font-family:${FONTS.body};font-size:${eyebrowSize}px;font-weight:600;letter-spacing:0.10em;text-transform:uppercase;color:${eyebrowColour};line-height:${Math.round(eyebrowSize * 1.45)}px;">${eyebrow}</td>
            <td valign="middle" align="right"${editAttr(editable, "date_label")} style="font-family:${FONTS.body};font-size:${dateSize}px;font-weight:600;letter-spacing:0.10em;text-transform:uppercase;color:${eyebrowColour};line-height:${Math.round(dateSize * 1.45)}px;">${dateLabel}</td>
          </tr>
        </table>
      </td>
    </tr>
...
<tr><td bgcolor="${bg}" class="tg-pad-mobile" style="background-color:${bg};padding:52px 48px 56px;">
  <table role="presentation" border="0" cellspacing="0" cellpadding="0" width="100%">
    ...
    <tr>
      <td style="padding:0;">
        <h1${headlineClass}${editAttr(editable, "headline")} style="font-family:${FONTS.heading};font-size:${headlineSize}px;font-weight:800;letter-spacing:-0.03em;line-height:1.1;color:${fg};margin:0;mso-line-height-rule:exactly;">${headlineHtml}</h1>
      </td>
    </tr>
    ${subhead ? `
    <tr>
      <td style="padding:18px 0 0 0;">
        <p${subheadClass}${editAttr(editable, "subhead")}${richAttr(editable)} style="font-family:${FONTS.body};font-size:${subheadSize}px;font-weight:400;color:${fg};line-height:1.55;margin:0;opacity:0.92;">${subhead}</p>
      </td>
    </tr>` : ""}
    ${ctaRowHtml}
  </table>
