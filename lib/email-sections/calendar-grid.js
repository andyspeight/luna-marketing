...
const { escHtml, escAttr, safeUrl, fallback, scaleFont, editAttr } = require("./_helpers");

function renderItem(item, brand, editable, index) {
    ...
    <table role="presentation" border="0" cellspacing="0" cellpadding="0" width="100%" style="background-color:${DESIGN_TOKENS.bgSecondary};border-radius:8px;">
      <tr>
        ...
        <td valign="middle" bgcolor="${DESIGN_TOKENS.bgSecondary}" style="background-color:${DESIGN_TOKENS.bgSecondary};padding:14px 16px;border-radius:0 8px 8px 0;">
          ${title ? `<div${editAttr(editable, `items.${index}.title`)} style="font-family:${FONTS.heading};font-size:14px;font-weight:600;letter-spacing:-0.01em;color:${DESIGN_TOKENS.textPrimary};line-height:1.3;margin:0 0 3px 0;">${title}</div>` : ""}
          ${text ? `<div${editAttr(editable, `items.${index}.text`)} style="font-family:${FONTS.body};font-size:12px;color:${DESIGN_TOKENS.textSecondary};line-height:1.5;margin:0;">${text}</div>` : ""}
        </td>
      </tr>
    </table>
    ${linkClose}
...
function render(props = {}, brand, editable) {
  ...
  const subSize = scaleFont(13, props.sub_size);

  if (!items.length) return "";

  const itemsHtml = items.map((d, i) => renderItem(d, brand, editable, i)).filter(Boolean).join("");

  return `
<tr><td bgcolor="${DESIGN_TOKENS.bgPrimary}" class="tg-pad-mobile" style="background-color:${DESIGN_TOKENS.bgPrimary};padding:36px;">
  <div${editAttr(editable, "title")} style="font-family:${FONTS.heading};font-size:${titleSize}px;font-weight:700;letter-spacing:-0.02em;color:${DESIGN_TOKENS.textPrimary};line-height:1.2;margin:0 0 6px 0;">${title}</div>
  ${sub ? `<div${editAttr(editable, "sub")} style="font-family:${FONTS.body};font-size:${subSize}px;color:${DESIGN_TOKENS.textSecondary};line-height:1.5;margin:0 0 20px 0;">${sub}</div>` : `<div style="height:20px;line-height:20px;font-size:20px;">&nbsp;</div>`}
...
const schema = {
  ...
  fields: [
    ...
    { key: "sub_size", label: "Sub-line text size", type: "fontSize", optional: true },
