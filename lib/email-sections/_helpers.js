...
/**
 * On-canvas edit hook. When `editable` is true, returns a
 * ` data-tg-edit="<propPath>"` attribute string (note the leading space)
 * that the builder uses to map a click on the rendered element back to a
 * section prop. Returns "" otherwise, so real sends are unaffected.
 * `path` is escaped for safe use inside a double-quoted attribute.
 */
function editAttr(editable, path) {
  if (!editable || !path) return "";
  return ` data-tg-edit="${escAttr(String(path))}"`;
}
