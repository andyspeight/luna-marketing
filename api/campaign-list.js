...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const [postRecs, emailRecs, names, mockupRecs] = await Promise.all([
    ...
    // Mockups generated by Promote (Generated Images of type product-mockup,
    // named "… — promo mockup", scoped to this tenant). This is the closest
    // thing to an "image library" for Promote output.
    const mockups = mockupRecs.filter(rec => {
      const f = rec.fields || {};
      const tenants = f["Tenant"] || [];
      const name = String(f["Name"] || "").toLowerCase();
      return f["PNG URL"] && tenants.includes(tenant) && name.includes("promo mockup");
    }).map(rec => {
      const f = rec.fields || {};
