...
function render(props = {}, brand, editable) {
  ...
  // The head's mobile media query hard-clamps .tg-hero-headline /
  // .tg-hero-subhead font-size with !important (which beats the inline
  // size). When the user has explicitly chosen a size we drop that class
  // so their choice wins at every width (incl. the ~600px builder
  // preview); when left on default we keep it for the usual responsive
  // shrink.
  const headlineClass = props.headline_size ? "" : ' class="tg-hero-headline"';
  const subheadClass = props.subhead_size ? "" : ' class="tg-hero-subhead"';

  // Author-written text colours default to the original treatment so
  // existing emails are unchanged; either can be overridden with a valid hex.
  const headingColour = safeHex(props.heading_colour) || BRAND.ink;
  const bodyColour = safeHex(props.body_colour) || BRAND.inkSoft;
...
<div${headlineClass}${editAttr(editable, "headline")} style="font-family:${FONTS.heading};font-size:${headlineSize}px;font-weight:700;color:${headingColour};line-height:${Math.round(headlineSize * (40 / 32))}px;margin:0;">${headline}</div>`;

  if (subhead) {
    html += `
<div${subheadClass}${editAttr(editable, "subhead")} style="font-family:${FONTS.body};font-size:${subheadSize}px;color:${bodyColour};line-height:${Math.round(subheadSize * (26 / 17))}px;margin:12px 0 0 0;">${subhead}</div>`;
  }
  html += `
</td></tr>`;
...
<a href="${ctaUrl}"${editAttr(editable, "ctaText")} style="display:inline-block;padding:14px 28px;font-family:${FONTS.body};font-size:15px;font-weight:600;color:${ctaFg};text-decoration:none;border-radius:8px;">${ctaText}</a>
</td></tr></table>
</td></tr>`;
  } else {
    html += `
...
const schema = {
  ...
  fields: [
    ...
    { key: "body_colour", label: "Subhead colour", type: "colour", optional: true, defaultPreview: "#334155", placeholder: "Defaults to current subhead colour", help: "Colour of the subhead text. Leave empty to keep the default." },
