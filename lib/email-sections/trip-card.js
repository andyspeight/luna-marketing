...
function render(props = {}, brand) {
  ...
  if (imageUrl) {
    const overlayHtml = `
      ${imageTag ? `<div style="display:inline-block;padding:4px 10px;background:rgba(255,255,255,0.15);border:1px solid rgba(255,255,255,0.25);border-radius:999px;font-family:${FONTS.body};font-size:${imageTagSize}px;font-weight:600;letter-spacing:0.08em;text-transform:uppercase;color:#ffffff;margin-bottom:8px;">${imageTag}</div>` : ""}
      <div style="font-family:${FONTS.heading};font-size:${imageNameSize}px;font-weight:700;letter-spacing:-0.02em;line-height:${Math.round(imageNameSize * (26 / 22))}px;color:#ffffff;margin:0;">${imageName}</div>`;
