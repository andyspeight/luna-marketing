...
function renderDetailRow(detail, brand, idx, editable) {
    ...
    <table role="presentation" class="tg-stack-mobile" border="0" cellspacing="0" cellpadding="0" width="100%">
      <tr>
        <td width="130" valign="top" style="padding-right:16px;">
          <div${editAttr(editable, `details.${idx}.label`)} style="font-family:${FONTS.body};font-size:12px;font-weight:500;letter-spacing:0.04em;text-transform:uppercase;color:${DESIGN_TOKENS.textTertiary};line-height:18px;">${label}</div>
        </td>
        <td valign="top">
          <div${editAttr(editable, `details.${idx}.value`)} style="font-family:${FONTS.body};font-size:14px;font-weight:600;color:${DESIGN_TOKENS.textPrimary};line-height:21px;">${value}</div>
...
function render(props = {}, brand, editable) {
  ...
  // Per-text-area sizes for the overlay tag/name. Bases match the px values
  // used in the overlay markup below.
  const imageTagSize = scaleFont(10, props.image_tag_size);
  const imageNameSize = scaleFont(22, props.image_name_size);
  ...
  if (imageUrl) {
    const overlayHtml = `
      ${imageTag ? `<div${editAttr(editable, "image_tag")} style="display:inline-block;padding:4px 10px;background:rgba(255,255,255,0.15);border:1px solid rgba(255,255,255,0.25);border-radius:999px;font-family:${FONTS.body};font-size:${imageTagSize}px;font-weight:600;letter-spacing:0.08em;text-transform:uppercase;color:#ffffff;margin-bottom:8px;">${imageTag}</div>` : ""}
      <div${editAttr(editable, "image_name")} style="font-family:${FONTS.heading};font-size:${imageNameSize}px;font-weight:700;letter-spacing:-0.02em;line-height:${Math.round(imageNameSize * (26 / 22))}px;color:#ffffff;margin:0;">${imageName}</div>`;
...
<!--<![endif]-->`;
  ...
  const detailRowsHtml = details.map((d, i) => renderDetailRow(d, brand, i, editable)).join("");
...
const schema = {
  ...
  fields: [
    ...
    { key: "image_tag_size", label: "Region tag text size", type: "fontSize", optional: true },
    { key: "image_name", label: "Destination name", type: "text", required: true, maxLength: 60 },
    { key: "image_name_size", label: "Destination name text size", type: "fontSize", optional: true },
