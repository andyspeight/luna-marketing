...
SOCIAL OUTPUT RULES — APPLY ONLY WHEN OUTPUT CONTEXT IS "social"

When the input declares OUTPUT CONTEXT: social, the image displays in a feed at roughly 500px wide on desktop and 350px on a phone — SMALLER than email. A full landing-page design (nav bar, hero, stats strip, dashboard cards, footer) squeezed onto the canvas becomes an unreadable blur at feed size. Design a POSTER, not a page:

S1. One message. A single focal headline is the whole job. On a 1200px-wide canvas: headline 64px or larger, one supporting line 28px or larger, and NOTHING on the canvas below 22px. If a piece of text cannot earn that size, cut it — do not shrink it.

S2. Three visual elements maximum: the headline block, ONE product visual moment (a single card, panel, or screenshot fragment — never a grid of several), and one accent (a chip, a single stat, or a CTA pill). Absolutely no nav bars, footers, code snippets, URL bars, multi-card dashboards, or multi-section page layouts.

S3. Big flat shapes and generous whitespace. Email rules E2 (single shadows) and E4 (no micro-detail) apply doubly at feed size.

S4. If a number is the story (a count, a price, a stat), make it the hero: 96px or larger, not a row in a stats strip.

S5. LIGHT backgrounds by default. Social feeds are bright white/light-grey environments; a dark navy canvas reads as a murky hole in the feed and its detail is hard to see at phone size. Default to a light surface — white, #F8FAFC, or a soft paper-to-teal-tint gradient — with navy (#1B2B5B) for text and teal (#00B4D8) for accents. The dark navy editorial treatment is the EXCEPTION, not the default: use it at most one time in four, and only when the content genuinely calls for drama (a bold launch statement, a night-themed message). If the previous image was dark, this one is light.
...
P8. PRODUCT-CENTRED COMPOSITION FOR EMAIL OUTPUT.
    ...
    Compose as follows: the product (Luna Chat widget, whichever product the spec describes) rendered at its full natural proportions, sized so it occupies between 60% and 75% of the canvas height. Centred horizontally on the canvas. The background is a clean brand-tinted surface. For EMAIL output the default is a soft navy gradient running diagonally from #1B2B5B at one corner to #0F1F4E at the opposite corner (a flat #0F1F4E or a subtle teal-to-navy gradient also work). For SOCIAL output, follow rule S5: default to a LIGHT studio surface (white, #F8FAFC, or a soft paper-to-teal tint) with the product's own navy providing the contrast — the dark navy studio is the occasional exception, not the norm. Choose ONE clean background, no overlays, no patterns, no textures.
...
function buildUserMessage(input, matchedProduct, hasScreenshot) {
  ...
  if (outputContext === "email") {
    ...
    lines.push("(This image will display at ~500px in a social feed, ~350px on a phone — SMALLER than email. Apply social rules S1-S4 from the system prompt: poster composition, one message, three elements max, no page layouts.)");
  }
  lines.push("");
