...
//
// Author: Travelgenix
// Date:   18 May 2026 (Stage C — adds generated-image guidance)
...
function formatSectionForPrompt(schema) {
  ...
  if (Array.isArray(schema.fields) && schema.fields.length > 0) {
    ...
    schema.fields.forEach(f => {
      // Internal/hidden fields the composer should never set directly.
      if (f.type === "hidden") return;
      const req = f.required ? "REQUIRED" : "optional";
      const typ = f.type || "string";
      let line = `  - ${f.key} (${typ}, ${req})`;
      if (f.maxLength) line += ` max ${f.maxLength}`;
      if (Array.isArray(f.options) && f.options.length) line += ` options: ${f.options.join(" | ")}`;
  ...
  // Surface composer hints inline so the model sees them right next to the schema
  if (schema.composerHints) {
    if (Array.isArray(schema.composerHints.whenToUse) && schema.composerHints.whenToUse.length > 0) {
      lines.push("When to use:");
      schema.composerHints.whenToUse.forEach(w => lines.push(`  - ${w}`));
    }
    if (Array.isArray(schema.composerHints.whenNotToUse) && schema.composerHints.whenNotToUse.length > 0) {
      lines.push("When NOT to use:");
      schema.composerHints.whenNotToUse.forEach(w => lines.push(`  - ${w}`));
    }
  }
  ...
  "imagesNeeded": [
    {
      ...
      "facts": ["string array — facts that must appear verbatim in the image"]
    }
  ],
  "warnings": [
...
# IMAGE GENERATION — TWO PATTERNS

The image pipeline supports two patterns. Choose deliberately and explain in reasoning.

## Pattern A: Image inside an existing section (hero, hero-image, feature-story, destination-grid card, two-column cell)

Set the image URL field to null. Add an entry to imagesNeeded describing the image. The user clicks Generate per-slot and the URL is dropped in.

## Pattern B: A generated-image section as its own block

Use the section type "generated-image" when the visual IS the message and would beat HTML composition. The image fills the section; there is no surrounding copy in the same section. The composer MUST still populate the imagesNeeded entry with detailed facts (verbatim strings that must appear in the image — prices, tier names, stats).

When you propose a generated-image section:
- Set props.imageUrl to null
- Set props.imageAlt to a clear description
- Set props.width and props.height to the slot's pixel dimensions (e.g. "1200" / "600" for hero)
- Add an imagesNeeded entry with style "product-mockup" or "abstract" and a populated "facts" array
- Justify the choice in reasoning ("Used generated-image for the pricing block because the three-tier comparison reads better as a single visual than as inline copy")

Do NOT use generated-image:
- For lifestyle/photography subjects (use a hero with style:"lifestyle" instead — those route to Pexels)
- When the user is likely to want to edit the text after composition (text is baked into the PNG)
- As a substitute for header, footer, or any structural section

# IMAGE STYLE GUIDANCE (use the right "style" tag for each imagesNeeded entry):

- "product-mockup": Apple-style SVG mockups of UI screens, dashboards, chat widgets, pricing tables, stat blocks. Use this when the image should show a Travelgenix product (Luna Chat, Luna Marketing, Travelify, Quick Quote, etc) in action, OR when the image is a designed editorial visual (pricing comparison, feature grid, data card). AI generates these well.
...
# FACTS IN IMAGES (CRITICAL)

For any image with style "product-mockup" or "abstract", include a "facts" array in the imagesNeeded entry listing any text that MUST appear verbatim in the generated image — prices, tier names, percentages, stat labels. The pipeline reproduces these strings exactly. Do not include facts unless you are certain they are correct (drawn from the Product Library or user input). Do not invent prices or stats here either — the fact-fidelity rule applies inside imagesNeeded just as it applies inside sections.
...
\`\`\`
{
  ...
  "dimensions": "hero",
  "suggestion": "Three-tier pricing card on a navy background. Spark on the left, Boost centred and highlighted as most popular, Ignite on the right. Clean typography, teal accents on the highlighted tier.",
  "facts": [
    "Spark £159/mo, setup £2,995",
    "Boost £229/mo, setup £2,995, most popular",
    "Ignite £299/mo, setup £3,995"
  ]
}
\`\`\`
  ...
  draft.sections.forEach((section, idx) => {
    ...
    if (schema && Array.isArray(schema.fields)) {
      ...
      schema.fields.forEach(field => {
        // Hidden fields are internal — don't validate required-ness
    ...
    // generated-image specific: if there's no imageUrl, there must be a
    // matching imagesNeeded entry. Otherwise this section will render as
    // an empty placeholder.
    if (section.type === "generated-image") {
      const props = section.props || {};
      const hasUrl = props.imageUrl && typeof props.imageUrl === "string" && props.imageUrl.length > 0;
      const hasBrief = Array.isArray(draft.imagesNeeded)
        && draft.imagesNeeded.some(b => b.sectionIndex === idx);
      if (!hasUrl && !hasBrief) {
        warnings.push(`Section ${idx} (generated-image): no imageUrl and no imagesNeeded entry — user will see a placeholder until they generate or paste an image.`);
      }
    }
  });
