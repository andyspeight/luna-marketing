...
const { escHtml, fallback, scaleFont, editAttr, safeHex } = require("./_helpers");
...
function renderFeatureRow(feature, isLast, brand, editable, index, headingColour, bodyColour) {
  ...
  // Row heading/body colours fall back to the design-token defaults so
  // existing emails are unchanged.
  const rowHeadingColour = headingColour || DESIGN_TOKENS.textPrimary;
  const rowBodyColour = bodyColour || DESIGN_TOKENS.textSecondary;
  ...
  <td style="padding:24px 0;${borderStyle}">
    <table role="presentation" border="0" cellspacing="0" cellpadding="0" width="100%">
      <tr>
        ...
        <td valign="top">
          ${heading ? `<div${editAttr(editable, `features.${index}.heading`)} style="font-family:${FONTS.heading};font-size:18px;font-weight:700;letter-spacing:-0.015em;color:${rowHeadingColour};line-height:1.3;margin:4px 0 6px 0;">${heading}</div>` : ""}
          ${text ? `<div${editAttr(editable, `features.${index}.text`)} style="font-family:${FONTS.body};font-size:14px;line-height:1.6;color:${rowBodyColour};margin:0;">${text}</div>` : ""}
        </td>
      </tr>
    </table>
  </td>
...
function render(props = {}, brand, editable) {
  ...
  // User-overridable text colours. Fall back to the existing design tokens
  // so emails without these props render byte-identically. The heading /
  // body colours apply to the header title/lede AND to every feature row.
  ...
  const featuresHtml = features
    .map((f, i) => renderFeatureRow(f, i === features.length - 1, brand, editable, i, headingColour, bodyColour))
    .filter(Boolean)
    .join("");
  ...
  <table role="presentation" border="0" cellspacing="0" cellpadding="0" width="100%">
    ...
    <tr>
      <td align="center" style="padding:0 0 12px 0;">
        <span${editAttr(editable, "eyebrow")} style="font-family:${FONTS.body};font-size:${eyebrowSize}px;font-weight:700;letter-spacing:0.12em;text-transform:uppercase;color:${eyebrowColour};line-height:${Math.round(eyebrowSize * (14 / 11))}px;">${eyebrow}</span>
      </td>
    </tr>` : ""}
    ${title ? `
    <tr>
      <td align="center" style="padding:0 0 12px 0;">
        <h2${editAttr(editable, "title")} style="font-family:${FONTS.heading};font-size:${titleSize}px;font-weight:700;line-height:1.2;letter-spacing:-0.025em;color:${headingColour};margin:0;">${title}</h2>
      </td>
    </tr>` : ""}
    ${lede ? `
    <tr>
      <td align="center" style="padding:0 0 40px 0;">
        <p${editAttr(editable, "lede")} style="font-family:${FONTS.body};font-size:${ledeSize}px;line-height:1.6;color:${bodyColour};margin:0 auto;max-width:460px;text-align:center;">${lede}</p>
      </td>
...
const schema = {
  ...
  fields: [
    ...
    { key: "heading_colour", label: "Heading colour", type: "colour", optional: true, defaultPreview: "#0F172A", help: "Leave empty to keep the default. Applies to the title and every feature heading." },
    ...
    { key: "body_colour", label: "Text colour", type: "colour", optional: true, defaultPreview: "#475569", help: "Leave empty to keep the default. Applies to the lede and every feature description." },
